...
enum PhaseImpl {
    Init(StateWaiter), 
    Publishing(StateWaiter), 
    Pushing(StateWaiter), 
    Uploading(StateWaiter), 
    Finished, 
    Error(BuckyError), 
}
...
pub trait BackupSrcDelegate: Send + Sync {
    ...
    async fn on_pre_upload_file(&self, session: &BackupSrcSession, rel_path: &str, task_path: &str);
...
impl BackupSrcSession {
    fn new<T: 'static + BackupSrcDelegate>(
        ...
        Self(Arc::new(SessionImpl {
            ...
            phase: RwLock::new(PhaseImpl::Init(StateWaiter::new()))
        }))
    }
    ...
    pub fn state(&self) -> BackupSrcPhase {
        let phase = self.0.phase.read().unwrap();
        match &*phase {
            PhaseImpl::Init(_) => BackupSrcPhase::Init, 
            PhaseImpl::Publishing(_) => BackupSrcPhase::Running,  
            PhaseImpl::Pushing(_) => BackupSrcPhase::Running, 
            PhaseImpl::Uploading(_) => BackupSrcPhase::Running, 
            PhaseImpl::Finished => BackupSrcPhase::Finished, 
            PhaseImpl::Error(err) => BackupSrcPhase::Error(err.clone()), 
        }
    }
    ...
    pub fn start(&self) -> BuckyResult<()> {
        ...
        let next = {
            ...
            match &mut *phase {
                PhaseImpl::Init(waiter) => {
                    Ok(if let Some(dir_id) = dir_id {
                        let next = NextStep::Push(dir_id);
                        *phase = PhaseImpl::Pushing(waiter.transfer());
                        next 
                    } else {    
                        *phase = PhaseImpl::Publishing(waiter.transfer());
                        NextStep::Publish
                    })
                }, 
    ...
    pub async fn wait_finish(&self) -> BackupSrcPhase {
        enum NextStep {
            Wait(AbortRegistration), 
            Return(BackupSrcPhase)
        }

        let next = {
            let mut phase = self.0.phase.write().unwrap();
            match &mut *phase {
                PhaseImpl::Init(waiter) => NextStep::Wait(waiter.new_waiter()), 
                PhaseImpl::Publishing(waiter) => NextStep::Wait(waiter.new_waiter()), 
                PhaseImpl::Pushing(waiter) => NextStep::Wait(waiter.new_waiter()), 
                PhaseImpl::Uploading(waiter) => NextStep::Wait(waiter.new_waiter()), 
                PhaseImpl::Finished => NextStep::Return(BackupSrcPhase::Finished), 
                PhaseImpl::Error(err) => NextStep::Return(BackupSrcPhase::Error(err.clone())) 
            }
        };
       
        match next {
            NextStep::Wait(waiter) => StateWaiter::wait(waiter, || self.state()).await,
            NextStep::Return(result) => result 
        }
    }

    pub async fn cancel_by_error(&self, err: BuckyError) {
        enum NextStep {
            None, 
            Wake(StateWaiter), 
            Cancel(StateWaiter)
        ...
        let next = {
            ...
            match &mut *phase {
                ...
                PhaseImpl::Publishing(waiter) => {
                    let next = NextStep::Wake(waiter.transfer());
                    *phase = PhaseImpl::Error(err.clone());
                    next
                }, 
                PhaseImpl::Pushing(waiter) => {
                    let next = NextStep::Wake(waiter.transfer());
                    *phase = PhaseImpl::Error(err.clone());
                    next
                }, 
                PhaseImpl::Uploading(waiter) => {
                    let next = NextStep::Cancel(waiter.transfer());
                    *phase = PhaseImpl::Error(err.clone());
                    next
                }, 
                PhaseImpl::Finished => NextStep::None, 
                PhaseImpl::Error(_) => NextStep::None 
            }
        };
        ...
        match next {
            ...
            NextStep::Wake(waiter) => {
                self.delegate().on_error(self, err).await;
                waiter.wake();
            }, 
            NextStep::Cancel(waiter) => {
    ...
    async fn push(&self, dir_id: ObjectId) {
        ...
        match self.push_inner(&dir_id).await {
            Ok(_) => {
                ...
                match &mut *phase {
                    PhaseImpl::Pushing(waiter) => {
                        *phase = PhaseImpl::Uploading(waiter.transfer());
                    },
                    _ => {}
                }
            },
    ...
    async fn on_interest(&self, _chunk: &ChunkId, rel_path: &str, task_path: &str) -> BuckyResult<InterestUploadSource> {
        ...
        if chunk_index == 0 {
            self.delegate().on_pre_upload_file(self, rel_path.as_os_str().to_str().unwrap(), task_path).await;
        }
    ...
    async fn on_post_publish(&self, session: &BackupSrcSession) {
        ...
        if let Some(exists) = exists {
            if !exists.ptr_eq(session) {
                let _ = exists.cancel_by_error(BuckyError::new(BuckyErrorCode::UserCanceled, "user canceled")).await;
            } else {
                return;
            }
        }
