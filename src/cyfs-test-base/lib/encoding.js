...
exports.EncodingError = exports.Encoding = void 0;
/**
...
class Encoding {
    ...
    static readI64(data, off) {
        const hi = data.readInt32LE(off + 4);
        const lo = data.readUInt32LE(off);
...
function write64(dst, num, off, be) {
    ...
    if (be) {
        off = dst.writeInt32BE(hi, off);
        off = dst.writeInt32BE(lo, off);
    }
    else {
        off = dst.writeInt32LE(lo, off);
        off = dst.writeInt32LE(hi, off);
    }
    return off;
}
/**
