...
export class BufferWriter {
        ...
        for (const op of this.ops) {
            switch (op.type) {
                case SEEK:
                    ...
                    break;
                case UI8:
                    off = data.writeUInt8(op.value, off);
                    break;
                case UI16:
                    off = data.writeUInt16LE(op.value, off);
                    break;
                case UI16BE:
                    off = data.writeUInt16BE(op.value, off);
                    break;
                case UI32:
                    off = data.writeUInt32LE(op.value, off);
                    break;
                case UI32BE:
                    off = data.writeUInt32BE(op.value, off);
                    break;
                case UI64:
                ...
                case UI64BE:
                    ...
                    break;
                case I8:
                    off = data.writeInt8(op.value, off);
                    break;
                case I16:
                    off = data.writeInt16LE(op.value, off);
                    break;
                case I16BE:
                    off = data.writeInt16BE(op.value, off);
                    break;
                case I32:
                    off = data.writeInt32LE(op.value, off);
                    break;
                case I32BE:
                    off = data.writeInt32BE(op.value, off);
                    break;
                case I64:
                ...
                case I64BE:
                    ...
                    break;
                case FL:
                    off = data.writeFloatLE(op.value, off);
                    break;
                case FLBE:
                    off = data.writeFloatBE(op.value, off);
                    break;
                case DBL:
                    off = data.writeDoubleLE(op.value, off);
                    break;
                case DBLBE:
                    off = data.writeDoubleBE(op.value, off);
                    break;
                case VARINT:
                ...
                case STR:
                    ...
                    break;
                case CHECKSUM:
                    off += Crypto.createHash('sha256').update(data.slice(0, off)).digest().copy(data, off, 0, 4);
                    break;
                case FILL:
    ...
    writeString(value: string|Buffer, enc?: BufferEncoding) {
        if (value.length === 0) {
            return;
        }
    ...
    writeVarString(value: string, enc?: BufferEncoding) {
        if (value.length === 0) {
    ...
    writeNullString(value: string|Buffer, enc?: BufferEncoding) {
...
class WriteOp {
    ...
    public enc: any;
    public size: any;
    constructor(type: any, value?: any, enc?: any, size?: any) {
        this.type = type;
        this.value = value;
        this.enc = enc;
        this.size = size;
