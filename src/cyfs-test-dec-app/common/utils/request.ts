...
export async function requestService(route: string, data: Object,target:ObjectId=stackInfo.serviceOOD) {
    return generateRequest(target)(route, data)
}

export async function requestLocal(route: string, data: Object) {
    return generateRequest(stackInfo.ood_device_id.object_id)(route, data)
}
...
export function generateRequest(target: ObjectId): (route: string, data: Object) => Promise<RequestTargetCommonResponse>{
    ...
    return async function (route: string, data: Object): Promise<RequestTargetCommonResponse> {
        ...
        const obj = GitTextObject.create(stackInfo.owner, stackInfo.appID, route, "header", dataString)
