 ...
 * 初始化模拟器测试程序
 */
   static async init(clear:boolean=false){
        if(clear){
            //(0) 清理模拟器的数据
   ...
   static async clearZoneSimulator(){
    ...
    static async getPeerId(){
        ...
        this.zone1_device1_peerId = strs[3].split(":")[1];
        this.zone1_device2_peerId = strs[4].split(":")[1];
        this.zone2_people = strs[7].split(":")[1];
        this.zone2_ood_peerId = strs[8].split(":")[1];
        this.zone2_device1_peerId = strs[9].split(":")[1];
        this.zone2_device2_peerId = strs[10].split(":")[1];    
    }
    
    static async connecStimulator() {
        this.zone1_ood_stack = cyfs.SharedCyfsStack.open(cyfs.SharedCyfsStackParam.new_with_ws_event_ports(simulator.zone1.ood.http_port, simulator.zone1.ood.ws_port).unwrap());
        this.zone1_device1_stack = cyfs.SharedCyfsStack.open(cyfs.SharedCyfsStackParam.new_with_ws_event_ports(simulator.zone1.device1.http_port, simulator.zone1.device1.ws_port).unwrap());
        this.zone1_device2_stack = cyfs.SharedCyfsStack.open(cyfs.SharedCyfsStackParam.new_with_ws_event_ports(simulator.zone1.device2.http_port, simulator.zone1.device2.ws_port).unwrap());
        // ZOne2 设备协议栈
        this.zone2_ood_stack = cyfs.SharedCyfsStack.open(cyfs.SharedCyfsStackParam.new_with_ws_event_ports(simulator.zone2.ood.http_port, simulator.zone2.ood.ws_port).unwrap());
        this.zone2_device1_stack = cyfs.SharedCyfsStack.open(cyfs.SharedCyfsStackParam.new_with_ws_event_ports(simulator.zone2.device1.http_port, simulator.zone2.device1.ws_port).unwrap());
        this.zone2_device2_stack = cyfs.SharedCyfsStack.open(cyfs.SharedCyfsStackParam.new_with_ws_event_ports(simulator.zone2.device2.http_port, simulator.zone2.device2.ws_port).unwrap());
        ...
        let res1 = await this.zone1_ood_stack.wait_online(cyfs.Some(JSBI.BigInt(6000)));
        if(res1.ok){
            connect = connect + 1;
            console.info('zone1_ood_stack 连接 wx http 成功')
        }else{
            console.info(`连接失败1：${res1.val?.msg}`)
            assert.equal(res1.val!.code,"0")
        }
        //等待 zone1_device1_stack 连接成功
        res1 = await this.zone1_device1_stack.wait_online(cyfs.Some(JSBI.BigInt(6000)));
        if(res1.ok){
            connect = connect + 1;
        ...
        res1 = await this.zone1_device2_stack.wait_online(cyfs.Some(JSBI.BigInt(6000)));
        if(res1.ok){
            connect = connect + 1;
        ...
        res1 = await this.zone2_ood_stack.wait_online(cyfs.Some(JSBI.BigInt(6000)));
        if(res1.ok){
            connect = connect + 1;
        ...
        res1 = await this.zone2_device2_stack.wait_online(cyfs.Some(JSBI.BigInt(6000)));
        if(res1.ok){
            connect = connect + 1;
        ...
        let res2 = await this.zone2_device1_stack.wait_online(cyfs.Some(JSBI.BigInt(6000)));
        if(res2.ok){
            connect = connect + 1;
