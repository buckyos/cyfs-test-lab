...
describe("#op-env 初始化方式", function () {
    ...
    describe.only("## r_meta测试执行", async () => {
        ...
        describe("## SharedCyfsStack r_meta测试", async () => {
            ...
            let gen_obj = async () => {
                return cyfs.TextObject.create(cyfs.ObjectId.from_base_58("5bnZHzYHCUJFRxXBAPDw8727uHdS95yhFnBmoNrwm7CP").unwrap(), `A${common_1.RandomGenerator.string(10)}`, `A${common_1.RandomGenerator.string(10)}`, `A${common_1.RandomGenerator.string(10)}`);
            };
            class PostObjectHandlerTest {
                async call(param) {
                    console.info("recv post request: {}", param.request.object.object_id);
                    const result = {
                        action: cyfs.RouterHandlerAction.Response,
                        request: undefined,
                        response: cyfs.Ok({ object: undefined })
                    };
            ...
            describe("## create_path_op_env root-state同zone同dec", async () => {
                it("create_path_op_env root-state同zone同dec,access,dec_default()权限组", async () => {
                    ...
                    let obj_id1 = (await gen_obj()).desc().object_id();
                    let key1 = common_1.RandomGenerator.string(10);
                    ...
                    assert_1.default.ok(!update_result2.err);
                    ...
                    assert_1.default.ok(!(await op_env2.get_by_key(path1, key1)).err);
                });
                ...
                it("create_path_op_env root-state同zone同dec,access,constructor()权限组初始化", async () => {
                    ...
                    let stub_b = stack_b.root_state_stub();
                    await stack_a.root_state_meta_stub().clear_access();
                    let acc = (new cyfs.AccessString(0o777));
                    await stack_a.root_state_meta_stub().add_access(cyfs.GlobalStatePathAccessItem.new(path1, acc));
                    let path_acc = {
                        path: path1,
                        access: cyfs.AccessPermissions.Full
            ...
            describe("## create_path_op_env root-state跨zone同dec", async () => {
                it("create_path_op_env root-state跨zone同dec,access,dec_default()权限组", async () => {
                    ...
                    // 添加一个Post Handler
                    {
                        ...
                        const ret = await ood2.router_handlers().add_post_object_handler(cyfs.RouterHandlerChain.Handler, 'post-object-handler', 0, undefined, path_handler.toString(), cyfs.RouterHandlerAction.Default, new PostObjectHandlerTest());
            ...
            describe("## SharedCyfsStack.RequestGlobalStatePath，req_path", async () => {
                it.only("req_path GlobalRoot|Decroot", async () => {
                    ...
                    console.info(`stack_b,${stack_b.dec_id}`);
                    let obj_id1 = (await gen_obj()).desc().object_id();
                    let obj_id2 = (await gen_obj()).desc().object_id();
                    let key1 = common_1.RandomGenerator.string(10);
                    let path1 = `/qatest/${key1}/${obj_id1}`;
                    ...
                    assert_1.default.ok(!update_result1.err);
                    ...
                    {
                        //const handler = new PostObjectHandler();
                        const ret = await stack_b.router_handlers().add_post_object_handler(cyfs.RouterHandlerChain.Handler, 'post-object-handler', 0, undefined, path_handler.toString(), cyfs.RouterHandlerAction.Default, new PostObjectHandlerTest());
                        console.info("ret2", ret);
                    }
                    // 测试stack1 Post给 stack2
