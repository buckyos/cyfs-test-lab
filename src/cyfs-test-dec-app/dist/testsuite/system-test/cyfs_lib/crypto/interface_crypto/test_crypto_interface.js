...
describe("SharedCyfsStack crypto目录", function () {
    ...
    describe("#crypto 相关接口", async () => {
        ...
        it("crypto 调用 sign_object系统授权、verify_object无需授权验证成功", async () => {
            ...
            let permission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/crypto/sign_object/", zone1device1.local_device_id().object_id, undefined, undefined, cyfs.AccessPermissions.Full);
        ...
        it("crypto 调用 sign_object系统授权，添加preCryptoHandler", async () => {
            let permission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/handler/pre_crypto/sign_object/", undefined, undefined, dec_app_1, cyfs.AccessPermissions.Full);
            await system_stack.root_state_meta_stub(system_stack.local_device_id().object_id, sysdec).add_access(permission);
            const obj = cyfs.TextObject.create(zone1device1.local_device_id().object_id, 'question_saveAndResponse', `test_header, time = ${Date.now()}`, `hello! time = ${Date.now()}`);
            const object_id = obj.desc().calculate_id();
            // 添加req_path       
            ...
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret1 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.SignObject, cyfs.RouterHandlerChain.PreCrypto, "post-object-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Default, myHandler.SignObjectHandlerDefault, "CryptoHandlerDefault", 1);
            assert(!ret1.err, `添加handler错误 ---> ${ret1}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
            let spermission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/crypto/sign_object/", undefined, undefined, dec_app_1, cyfs.AccessPermissions.Full);
        ...
        it("crypto 调用 sign_object系统授权，添加postCryptoHandler", async () => {
            let permission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/handler/post_crypto/sign_object/", undefined, undefined, dec_app_1, cyfs.AccessPermissions.Full);
            await system_stack.root_state_meta_stub(system_stack.local_device_id().object_id, sysdec).add_access(permission);
            const obj = cyfs.TextObject.create(zone1device1.local_device_id().object_id, 'question_saveAndResponse', `test_header, time = ${Date.now()}`, `hello! time = ${Date.now()}`);
            const object_id = obj.desc().calculate_id();
            // 添加req_path       
            ...
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret1 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.SignObject, cyfs.RouterHandlerChain.PostCrypto, "post-object-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Default, myHandler.SignObjectHandlerDefault, "CryptoHandlerDefault", 1);
            assert(!ret1.err, `添加handler错误 ---> ${ret1}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
            let spermission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/crypto/sign_object/", undefined, undefined, dec_app_1, cyfs.AccessPermissions.Full);
            await system_stack.root_state_meta_stub(system_stack.local_device_id().object_id, sysdec).add_access(spermission);
        ...
        it.skip("crypto 调用 系统已授权选择people签名", async () => {
            let permission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/crypto/sign_object/", zone1device1.local_device_id().object_id, cyfs.DeviceZoneCategory.CurrentZone, dec_app_1, cyfs.AccessPermissions.Full);
        ...
        it("crypto device2调用 encrypt_data、decrypt_data EncryptData", async () => {
            ...
            let ob = common_1.RandomGenerator.get_len_buf(48);
        ...
        it("crypto调用 encrypt_data,数据长度最大117bytes", async () => {
            ...
            // const object_id = obj.desc().calculate_id();
            // const ob = obj.to_vec().unwrap()
            let ob = common_1.RandomGenerator.get_len_buf(117);
            console.log("----------------------=====================================:  " + ob.byteLength);
            console.info(`will encrypt_data: EncryptData `);
            const enreq = {
                common: {
        ...
        it("crypto调用 encrypt_data,数据长度1bytes", async () => {
            ...
            // const object_id = obj.desc().calculate_id();
            // const ob = obj.to_vec().unwrap()
            let ob = common_1.RandomGenerator.get_len_buf(1);
            console.log("----------------------=====================================:  " + ob.byteLength);
            console.info(`will encrypt_data: EncryptData `);
            const enreq = {
                common: {
        ...
        it("crypto调用 encrypt_data GenAESKeyAndEncrypt PreCrypto", async () => {
            let permission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/handler/pre_crypto/encrypt_data/", undefined, undefined, dec_app_1, cyfs.AccessPermissions.Full);
            await system_stack.root_state_meta_stub(system_stack.local_device_id().object_id, sysdec).add_access(permission);
            // 添加req_path       
            let path = "/test_non/reqpath/";
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.EncryptData, cyfs.RouterHandlerChain.PreCrypto, "encrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Default, myHandler.EncryptDataHandlerDefault, "EncryptDataHandlerDefault", 1);
        ...
        it("crypto调用 encrypt_data GenAESKeyAndEncrypt PostCrypto", async () => {
            let permission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/handler/post_crypto/encrypt_data/", undefined, undefined, dec_app_1, cyfs.AccessPermissions.Full);
            await system_stack.root_state_meta_stub(system_stack.local_device_id().object_id, sysdec).add_access(permission);
            // 添加req_path       
            let path = "/test_non/reqpath/";
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.EncryptData, cyfs.RouterHandlerChain.PostCrypto, "encrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Default, myHandler.EncryptDataHandlerDefault, "EncryptDataHandlerDefault", 1);
            assert(!ret2.err, `添加handler错误 ---> ${ret2}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
            console.info(`will encrypt_data: GenAESKeyAndEncrypt `);
            const enreq = {
        ...
        it("crypto调用 encrypt_data GenAESKeyAndEncrypt PreCrypto+action=pass", async () => {
            ...
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.EncryptData, cyfs.RouterHandlerChain.PreCrypto, "encrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Pass, myHandler.EncryptDataHandlerPass, "EncryptDataHandlerPass", 1);
            assert(!ret2.err, `添加handler错误 ---> ${ret2}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
            console.info(`will encrypt_data: GenAESKeyAndEncrypt `);
            const enreq = {
        ...
        it("crypto调用 encrypt_data GenAESKeyAndEncrypt PreCrypto+action=drop", async () => {
            ...
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.EncryptData, cyfs.RouterHandlerChain.PreCrypto, "encrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Drop, myHandler.EncryptDataHandlerDrop, "EncryptDataHandlerDrop", 1);
            assert(!ret2.err, `添加handler错误 ---> ${ret2}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
            console.info(`will encrypt_data: GenAESKeyAndEncrypt `);
            const enreq = {
        ...
        it("crypto调用 encrypt_data GenAESKeyAndEncrypt PreCrypto+action=reject", async () => {
            ...
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.EncryptData, cyfs.RouterHandlerChain.PreCrypto, "encrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Reject, myHandler.EncryptDataHandlerReject, "EncryptDataHandlerReject", 1);
            assert(!ret2.err, `添加handler错误 ---> ${ret2}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
            console.info(`will encrypt_data: GenAESKeyAndEncrypt `);
            const enreq = {
        ...
        it("crypto调用 encrypt_data GenAESKeyAndEncrypt PreCrypto+action=response", async () => {
            ...
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.EncryptData, cyfs.RouterHandlerChain.PreCrypto, "encrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Response, myHandler.EncryptDataHandlerResponse, "EncryptDataHandlerResponse", 1);
            assert(!ret2.err, `添加handler错误 ---> ${ret2}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
            console.info(`will encrypt_data: GenAESKeyAndEncrypt `);
            const enreq = {
        ...
        it("crypto调用 decrypt_data GenAESKeyAndEncrypt PreCrypto", async () => {
            let permission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/handler/pre_crypto/decrypt_data/", undefined, undefined, dec_app_1, cyfs.AccessPermissions.Full);
            await system_stack.root_state_meta_stub(system_stack.local_device_id().object_id, sysdec).add_access(permission);
            // 添加req_path       
            let path = "/test_non/reqpath/";
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.DecryptData, cyfs.RouterHandlerChain.PreCrypto, "decrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Default, myHandler.DecryptDataHandlerDefault, "DecryptDataHandlerDefault", 1);
            assert(!ret2.err, `添加handler错误 ---> ${ret2}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
            console.info(`will encrypt_data: GenAESKeyAndEncrypt `);
            const enreq = {
        ...
        it("crypto调用 decrypt_data GenAESKeyAndEncrypt PostCrypto", async () => {
            let permission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/handler/post_crypto/decrypt_data/", undefined, undefined, dec_app_1, cyfs.AccessPermissions.Full);
            await system_stack.root_state_meta_stub(system_stack.local_device_id().object_id, sysdec).add_access(permission);
            // 添加req_path       
            let path = "/test_non/reqpath/";
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.DecryptData, cyfs.RouterHandlerChain.PostCrypto, "decrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Default, myHandler.DecryptDataHandlerDefault, "DecryptDataHandlerDefault", 1);
            assert(!ret2.err, `添加handler错误 ---> ${ret2}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
            console.info(`will encrypt_data: GenAESKeyAndEncrypt `);
            const enreq = {
        ...
        it("crypto调用 decrypt_data GenAESKeyAndEncrypt PreCrypto+action=pass", async () => {
            ...
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.DecryptData, cyfs.RouterHandlerChain.PreCrypto, "decrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Pass, myHandler.DecryptDataHandlerPass, "DecryptDataHandlerPass", 1);
            assert(!ret2.err, `添加handler错误 ---> ${ret2}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
            console.info(`will encrypt_data: GenAESKeyAndEncrypt `);
            const enreq = {
        ...
        it("crypto调用 decrypt_data GenAESKeyAndEncrypt PreCrypto+action=response", async () => {
            ...
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.DecryptData, cyfs.RouterHandlerChain.PreCrypto, "decrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Response, myHandler.DecryptDataHandlerResponse, "DecryptDataHandlerResponse", 1);
            assert(!ret2.err, `添加handler错误 ---> ${ret2}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
            console.info(`will encrypt_data: GenAESKeyAndEncrypt `);
            const enreq = {
        ...
        it("crypto调用 decrypt_data GenAESKeyAndEncrypt PreCrypto+action=drop", async () => {
            ...
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.DecryptData, cyfs.RouterHandlerChain.PreCrypto, "decrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Drop, myHandler.DecryptDataHandlerDrop, "DecryptDataHandlerDrop", 1);
            assert(!ret2.err, `添加handler错误 ---> ${ret2}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
            console.info(`will encrypt_data: GenAESKeyAndEncrypt `);
            const enreq = {
        ...
        it("crypto调用 decrypt_data GenAESKeyAndEncrypt PreCrypto+action=reject", async () => {
            ...
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.DecryptData, cyfs.RouterHandlerChain.PreCrypto, "decrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Reject, myHandler.DecryptDataHandlerReject, "DecryptDataHandlerReject", 1);
            assert(!ret2.err, `添加handler错误 ---> ${ret2}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
            console.info(`will encrypt_data: GenAESKeyAndEncrypt `);
            const enreq = {
        ...
        it("crypto调用 encrypt_data GenAESKeyAndEncrypt PreCrypto", async () => {
            ...
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
            console.log("------------------------> " + req_path);
            const ret1 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.EncryptData, cyfs.RouterHandlerChain.PreCrypto, "encrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Default, myHandler.EncryptDataHandlerDefault, "EncryptDataHandlerDefault", 1);
            assert(!ret1.err, `添加handler错误 ---> ${ret1}`);
            let depermission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/handler/pre_crypto/decrypt_data/", undefined, undefined, dec_app_1, cyfs.AccessPermissions.Full);
            await system_stack.root_state_meta_stub(system_stack.local_device_id().object_id, sysdec).add_access(depermission);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.DecryptData, cyfs.RouterHandlerChain.PreCrypto, "decrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Default, myHandler.DecryptDataHandlerDefault, "DecryptDataHandlerDefault", 1);
            assert(!ret2.err, `添加handler错误 ---> ${ret2}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
        ...
        it("crypto调用 encrypt_data GenAESKeyAndEncrypt PostCrypto", async () => {
            ...
            const ret1 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.EncryptData, cyfs.RouterHandlerChain.PostCrypto, "encrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Default, myHandler.EncryptDataHandlerDefault, "EncryptDataHandlerDefault", 1);
            assert(!ret1.err, `添加handler错误 ---> ${ret1}`);
            let depermission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/handler/post_crypto/decrypt_data/", undefined, undefined, dec_app_1, cyfs.AccessPermissions.Full);
            await system_stack.root_state_meta_stub(system_stack.local_device_id().object_id, sysdec).add_access(depermission);
            const ret2 = await handlerManager.addHandler(`${zone1device1.local_device_id().to_base_58()}`, zone1device1, cyfs.RouterHandlerCategory.DecryptData, cyfs.RouterHandlerChain.PostCrypto, "decrypt-data-handler-001", 1, undefined, req_path, cyfs.RouterHandlerAction.Default, myHandler.DecryptDataHandlerDefault, "DecryptDataHandlerDefault", 1);
            assert(!ret2.err, `添加handler错误 ---> ${ret2}`);
            let check = handlerManager.startHandlerCheck(10 * 1000);
        ...
        it("crypto跨Dec设备调用 encrypt_data、decrypt_data EncryptData postCrypto", async () => {
            let hpermission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/handler/post_crypto/encrypt_data/", undefined, undefined, dec_app_1, cyfs.AccessPermissions.Full);
        ...
        it("crypto跨Dec设备调用 encrypt_data、decrypt_data EncryptData preCrypto", async () => {
            let hpermission = cyfs.GlobalStatePathAccessItem.new_group("/.cyfs/api/handler/pre_crypto/encrypt_data/", undefined, undefined, dec_app_1, cyfs.AccessPermissions.Full);
            await system_stack.root_state_meta_stub(system_stack.local_device_id().object_id, sysdec).add_access(hpermission);
            // 添加req_path       
            let path = "/test_non/reqpath/";
            let req_path = new cyfs.RequestGlobalStatePath(dec_app_1, path).toString();
        ...
        it("crypto  sign_and_push_named_object重复叠加多个签名和覆盖5次 ", async () => {
            let [people, people_pk] = dec_app_base_1.create_people_random();
            const people_id = people.calculate_id();
            const unique_id = Date.now().toString();
            const [ood, ood_pk, address_index] = dec_app_base_1.create_device(people_id, people_pk, cyfs.DeviceCategory.OOD, unique_id);
            // 设置People的ood_list
            people.body_expect().content().ood_list.push(ood.device_id());
            people.body_expect().increase_update_time(cyfs.bucky_time_now());
            const [runtime, runtime_pk] = dec_app_base_1.create_device(people_id, people_pk, cyfs.DeviceCategory.PC, unique_id, 'runtime');
            // People给ood签名
