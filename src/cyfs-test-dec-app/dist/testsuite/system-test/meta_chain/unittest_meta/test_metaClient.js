...
const meta_config = __importStar(require("./meta_config"));
...
function create_device(owner, cat) {
    let pk = cyfs.PrivateKey.generate_rsa(1024).unwrap();
    let public_key = pk.public();
    let unique = cyfs.UniqueId.copy_from_slice(stringToUint8Array(Date.now().toString()));
    let device = cyfs.Device.create(owner, unique, [], [], [], public_key, cyfs.Area.default(), cat);
    return [device, pk];
}
function load_desc_and_sec(path) {
...
function save_desc_and_sec(object, private_key, save_path) {
    ...
    describe("meta接口测试测试", async () => {
        describe("get_meta_miner_host 接口", async () => {
            it("target 为Dev 链", async () => {
                ...
                assert_1.default.ok(meta_config.check_array_contain(meta_config.Meta_miner_host_nightly, url), "meta_miner_host url不正确");
            });
            it("target Test 链", async () => {
                let url = cyfs.get_meta_miner_host(cyfs.MetaMinerTarget.Test);
                assert_1.default.ok(meta_config.check_array_contain(meta_config.Meta_miner_host_beta, url), "meta_miner_host url不正确");
            });
        });
        describe("get_meta_spv_host 接口", async () => {
            it("target 为Dev 链", async () => {
                let url = cyfs.get_meta_spv_host(cyfs.MetaMinerTarget.Dev);
                assert_1.default.equal(meta_config.Meta_spv_host_nightly, url, "get_meta_spv_host url不正确");
            });
            it("target 为Test 链", async () => {
                let url = cyfs.get_meta_spv_host(cyfs.MetaMinerTarget.Test);
                assert_1.default.equal(meta_config.Meta_spv_host_beta, url, "get_meta_spv_host url不正确");
            });
        });
        describe("meta_target_from_str 接口", async () => {
            it("target 为Dev 链", async () => {
                let url = cyfs.meta_target_from_str("dev");
                assert_1.default.equal(0, url, "meta_target_from_str url不正确");
            });
            it("target 为Test 链", async () => {
                let url = cyfs.meta_target_from_str("test");
                assert_1.default.equal(1, url, "meta_target_from_str url不正确");
            });
            it("target 为foemal 链", async () => {
                let url = cyfs.meta_target_from_str("formal");
                assert_1.default.equal(2, url, "meta_target_from_str url不正确");
            });
        });
        describe("get_meta_client 接口", async () => {
            it("target 为Dev 链", async () => {
                ...
                assert_1.default.ok((info === null || info === void 0 ? void 0 : info.err) == 0);
            });
            it("target 为Test 链", async () => {
        ...
        describe("create_meta_client 接口", async () => {
            ...
            it("参数 target_str、spv_str均为undefined ", async () => {
                let meta_client = cyfs.create_meta_client();
                let info = await meta_client.getSpvStatus();
                assert_1.default.ok((info === null || info === void 0 ? void 0 : info.err) == 0);
            });
            it("参数 spv_str为undefined ", async () => {
        ...
        describe("MetaClient  类接口 Nightly 环境", async () => {
            ...
            async function checkReceipt(txId, timeout = 300 * 1000) {
                console.info(txId);
                let time = Date.now();
            ...
            describe("MetaClient 类 getFileRewardAmount 接口", async () => {
                it("getFileRewardAmount 接口", async () => {
                    ...
                    console.info(JSON.stringify(info));
                    assert_1.default.ok(info.err == 0, `返回结果失败：${info}`);
                });
            });
            ...
            describe("MetaClient 类 getTx 接口", async () => {
                it("getTx 接口", async () => {
                    ...
                    console.info(JSON.stringify(info));
                    assert_1.default.ok(info.err == 0);
                });
            });
            ...
            describe("MetaClient 类 view_request 接口", async () => {
                it("view_request 接口", async () => {
                    const view = new cyfs.ViewRequest(cyfs.ViewBlockEnum.Tip(), cyfs.ViewMethodEnum.ViewDesc(new cyfs.ViewDescMethod(cyfs.ObjectId.from_base_58("5r4MYfFLxNDeW79UMpd9qXd5Xx6su25tXnMGBWbkEBBk").unwrap())));
                    let info = await meta_client_nightly.view_request(view);
                    console.info(JSON.stringify(info));
                    assert_1.default.ok(info.ok);
                });
            });
            ...
            describe("MetaClient 类 getName 接口", async () => {
                it("getName 接口 name被使用", async () => {
                    ...
                    console.info(JSON.stringify(info));
                    assert_1.default.ok(info.ok);
                    assert_1.default.ok(info.val.name_info.owner, "name 未被使用");
                });
            ...
            describe("MetaClient 类 create_desc 接口", async () => {
                ...
                it("create_desc 接口 上链 People对象", async () => {
                    ...
                    assert_1.default(!check.err, `${info.unwrap().to_base_58()} 检查是否上链超时`);
                    assert_1.default(check.receipt.result == 0, `${info.unwrap().to_base_58()} 上链失败，返回错误 ${JSON.stringify(check)}`);
                });
                it("create_desc 接口 上链 Device对象", async () => {
                    ...
                    assert_1.default(!check.err, `${info.unwrap().to_base_58()} 检查是否上链超时`);
                    assert_1.default(check.receipt.result == 0, `${info.unwrap().to_base_58()} 上链失败，返回错误 ${check.receipt}`);
                });
            ...
            describe("MetaClient 类 update_desc 接口", async () => {
                it("update_desc 接口", async () => {
                    let [people, people_pk] = load_desc_and_sec(path.join(__dirname, './people'));
                    let info = await meta_client_nightly.update_desc(people, cyfs.SavedMetaObject.try_from(people).unwrap(), 0, 0, people_pk);
                    console.info(JSON.stringify(info));
                    assert_1.default.ok(info.ok);
                    let check = await checkReceipt(info.unwrap().to_base_58());
                    assert_1.default(!check.err, `${info.unwrap().to_base_58()} 检查是否上链超时`);
                ...
                it("create_desc 接口 上链 People对象 update_desc 更新people ood list", async () => {
                    ...
                    assert_1.default(!check2.err, `${info2.unwrap().to_base_58()} 检查是否上链超时`);
                    assert_1.default(check2.receipt.result == 0, `${info2.unwrap().to_base_58()} 上链失败，返回错误 ${JSON.stringify(check2)}`);
                });
            });
        ...
        describe.skip("MetaClient  类接口 Beta 环境", async () => {
            ...
            describe("MetaClient 类 view_request 接口", async () => {
                it("view_request 接口", async () => {
                    const view = new cyfs.ViewRequest(cyfs.ViewBlockEnum.Tip(), cyfs.ViewMethodEnum.ViewDesc(new cyfs.ViewDescMethod(cyfs.ObjectId.from_base_58("5r4MYfFHhSRcVybjLNNTGvECDsW8yxej8U7YRqgTJxS2").unwrap())));
            ...
            describe("MetaClient 类 update_desc 接口", async () => {
                it("update_desc 接口", async () => {
                    let [people, people_pk] = load_desc_and_sec(path.join(__dirname, './people'));
                    let info = await meta_client_beta.update_desc(people, cyfs.SavedMetaObject.try_from(people).unwrap(), 0, 0, people_pk);
                    console.info(JSON.stringify(info));
                    assert_1.default.ok(info.ok);
                    let check = await checkReceipt(info.unwrap().to_base_58());
                    assert_1.default(!check.err, `${info.unwrap().to_base_58()} 检查是否上链超时`);
