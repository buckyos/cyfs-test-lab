...
describe("ood-daemon 本地1330服务测试", function () {
    ...
    describe("GET 获取OOD service 和 app-manager 数据", async () => {
        it("get_status", async () => {
            let get_data = await request_1.request("http://192.168.100.205:11330", "GET", "service_status");
            console.info(`${JSON.stringify(get_data)}`);
        });
        it.only("update status", async () => {
            let update = await request_1.request("http://192.168.100.205:11330", "POST", "service_status/9tGpLNnWHEvsnfTLErsuZVPYhnS6mZYfzStZx3VE4N4u", JSON.stringify({
                name: "9tGpLNnWHEvsnfTLErsuZVPYhnS6mZYfzStZx3VE4N4u",
            }), request_1.ContentType.raw);
