...
class BufferReader {
    ...
    assert(value) {
        if (!value) {
            throw new encoding_1.EncodingError(this.offset, 'Out of bounds read', assert_1.default);
        }
    }
    /**
     * Assertion.
     ...
     */
        ...
        assert_1.default.ok(this.stack.length > 0);
     ...
     */
    endData(zeroCopy) {
        assert_1.default(this.stack.length > 0);
        const start = this.stack.pop();
    ...
    readU16() {
        ...
        const ret = this.data.readUInt16LE(this.offset);
        this.offset += 2;
        return ret;
    }
    /**
    ...
    readU32() {
        ...
        const ret = this.data.readUInt32LE(this.offset);
        this.offset += 4;
        return ret;
    }
    /**
    ...
    readI16() {
        this.assert(this.offset + 2 <= this.data.length);
        const ret = this.data.readInt16LE(this.offset);
        this.offset += 2;
        return ret;
    }
    /**
    ...
    readI32() {
        this.assert(this.offset + 4 <= this.data.length);
        const ret = this.data.readInt32LE(this.offset);
        this.offset += 4;
        return ret;
    }
    /**
    ...
    readFloat() {
        this.assert(this.offset + 4 <= this.data.length);
        const ret = this.data.readFloatLE(this.offset);
        this.offset += 4;
        return ret;
    }
    /**
    ...
    readDouble() {
        this.assert(this.offset + 8 <= this.data.length);
        const ret = this.data.readDoubleLE(this.offset);
        this.offset += 8;
        return ret;
    }
    /**
    ...
    createChecksum() {
        ...
        return Crypto.createHash('sha256').update(data).digest().readUInt32LE(0);
    }
    /**
