...
class BufferWriter {
        ...
        for (const op of this.ops) {
            switch (op.type) {
                ...
                case FILL:
                    ...
                    break;
                default:
                    assert_1.default.ok(false, 'Bad type.');
                    break;
            }
        }
        assert_1.default(off === data.length, `off = ${off} data.length = ${data.length}`);
        this.destroy();
        return data;
    }
    /**
    ...
    writeHash(value) {
        if (typeof value !== 'string') {
            assert_1.default(value.length === 32);
            this.writeBytes(value);
            return;
        }
        assert_1.default(value.length === 64);
