...
exports.RandomGenerator = exports.IMGURL = exports.encodeType = exports.LANGUAGELIST = exports.testLanguage = exports.ESC_char = void 0;
...
exports.LANGUAGELIST = [cn, cn_t, en, jp, french, german, hanyu, xibanyayu, eyu, eyu, putaoyayu, yidaliyu, yuelanyu, yingninyu, alaboyu, taiyu];
/**
 * ASCII
 * UNICODE
 * ISO-8859-1
 ...
 * UTF-16
*/
exports.encodeType = ["ASCII", "UNICODE", "UTF-8", "UTF-16", "GBK", "GB18030", "GB2312", "ISO-8859-1"];
/**
 * 随机图片生成
 */
exports.IMGURL = [
...
class RandomGenerator {
    ...
    static async rand_cyfs_chunk_cache(chunk_size) {
        ...
        return { err: _1.ErrorCode.succ, chunk_data, chunk_id: chunk_calculate };
    }
    async rand_cyfs_file_cache(owner, file_size, chunk_size) {
        ...
        return { err: _1.ErrorCode.succ, file, file_data, md5 };
    }
    ...
    static string(length = 32, cn = 0, symbol = 0) {
    ...
    ;
    static accurate_string(length = 32) {
        let maxPos = RandomGenerator.CHAR_SET.length;
        let result = '';
        for (let i = 0; i < length; i++) {
            result += RandomGenerator.CHAR_SET.charAt(RandomGenerator.integer(maxPos));
    ...
    static language(length = 32, type = 2) {
        let myLen = 0;
        let result = "";
        while (myLen < length) {
            result += exports.LANGUAGELIST[type].charAt(RandomGenerator.integer(exports.LANGUAGELIST[type].length));
            myLen = myLen + 1;
        }
        return result;
    }
    static unicode(length) {
        return Array.from({ length }, () => String.fromCharCode(Math.floor(Math.random() * (65536)))).join('');
    }
    static accii(length) {
        return Array.from({ length }, () => String.fromCharCode(Math.floor(Math.random() * (255)))).join('');
    }
    static encode(length, type) {
     ...
     *  不建议使用该函数
     */
    static async create_random_file(pathDir, name, size) {
     ...
     *  不建议使用该函数
     */
    static compare_file_md5(sourcePath, targetPath) {
     ...
     *  不建议使用该函数
     */
    static async create_random_dir(root, dirNumber, fileNumber, fileSize, deep = 1) {
    ...
    static async img(img_path, name) {
        if (!fs.pathExistsSync(img_path)) {
            fs.mkdirpSync(img_path);
        }
        img_path = path.join(img_path, name);
        return new Promise(async (resolve) => {
            let url = exports.IMGURL[RandomGenerator.integer(exports.IMGURL.length - 1)];
    ...
    static async create_random_img(img_path, name) {
        // 获取一张图片
...
RandomGenerator.CN_SET = '巴克云网络科技有限公司测试汉字厸厶厽孓宀巛巜彳廴彡彐彳忄扌攵 氵灬 爫犭疒癶礻糹纟';
RandomGenerator.SYMBOL = 'iəɔua##$&@æ。？！.《》……&（）';
RandomGenerator.LANGUAGELIST = [cn, cn_t, en, jp, french, german, hanyu, xibanyayu, eyu, eyu, putaoyayu, yidaliyu, yuelanyu, yingninyu, alaboyu, taiyu];
;
