...
async function addDir(source: cyfs.SharedCyfsStack, target: cyfs.SharedCyfsStack, stack_manager: StackManager, fileSize: number, chunkSize: number, level: cyfs.NDNAPILevel, mount?: string, control_object?: string, access?: cyfs.AccessString, putdata?: string, mixchar?: boolean) {
    ...
    console.info(`put_context  =  ${JSON.stringify(info.unwrap())}`);
...
describe("CYFS Stack 磁盘满后noc、ndc、tracker读写测试", function () {
    ...
    describe(`验证磁盘写满请求: NDN 数据写入读取`, async () => {
        ...
        it(`put_data`, async () => {
            ...
            let randomStr = RandomGenerator.string(10 * 1024*1024);
    ...
    describe(`验证磁盘写满请求: tarns 数据写入读取`, async () => {
        ...
        it(`publish_file从文件中获取chunk`, async () => {
            ...
            // 创建测试文件
            let local_file = await file_source_tool.create_file(100 * 1024 * 1024);
            taskfile=(await file_source_tool.get_cache_path()).cache_path?.file_download
            file_name = local_file.file_name!
            file_path = local_file.file_path!
            let info = await stack.trans().publish_file({
                common: {
