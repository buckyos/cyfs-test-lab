...
// 使用nginx 代理测试
const service_url = `http://127.0.0.1:${1318}`;
const ws_url = `ws://127.0.0.1:${1319}`;
...
describe("cyfs-back-up数据恢复测试",function(){
    ...
    describe.only("cyfs-back-up测试数据构造", async () => {
        describe("NON nameobject 构造保存", async () => {
        ...
        describe ("Object Map 数据构造保存", async () => {
            ...
            it.skip("往root_state中插入ObjectMapOpEnvType.IsolatePath数据",async()=>{
                //let stack = stack_manager.get_cyfs_satck(test_device).stack!;
                let running = []
                for (let i = 0; i < ket_data_thread; i++) {
                    running.push(new Promise(async(resolve)=>{
            ...
            it.skip("往root_state中插入ObjectMapOpEnvType.Single数据",async()=>{
                //let stack = stack_manager.get_cyfs_satck(test_device).stack!;
                let running = []
                for (let i = 0; i < ket_data_thread; i++) {
                    running.push(new Promise(async(resolve)=>{
        ...
        describe.only("NON chunk 构造保存", async () => {
                ...
                for (let i = 0; i < chunk_thread; i++) {
                    running.push(new Promise(async(resolve)=>{
                        ...
                        for (let j = 0; j < chunk_number; j++) {
                            let begin = Date.now();
                            let tool =  new LocalUtilTool(logger,"E:\\local_data")
                ...
                for (let i = 0; i < file_thread; i++) {
                    running.push(new Promise(async(resolve)=>{
                        ...
                        for (let j = 0; j < file_number; j++) {
                            let begin = Date.now();
                            let tool =  new LocalUtilTool(logger,"E:\\local_data") //stack_manager.driver!.get_client(test_device.peer_name).client!.get_util_tool();
