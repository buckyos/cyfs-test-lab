...
class PackageCodec {
    ...
    encode(pkgs) { // []
        ...
        for (let i = 0; i < pkgs.length; i++) {
            ...
            const pkgBuffer = Buffer.from(JSON.stringify(pkg));
            pkgBuffer.copy(buffer, offset);
            offset += pkgBuffer.length;

            buffer.writeUInt32LE(pkgBuffer.length, 4);
        }
    ...
    _decode(buffer, offset) {
        ...
        let pkg = null;
        try {
            pkg = JSON.parse(pkgBuffer.slice(offset));
        } catch (error) {
