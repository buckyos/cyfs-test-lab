...
const {EndPoint} = require('../projects/bdt_echo/bdt/base/util.js');
...
class Stat{
    ...
    static makeConnectID(record) {
        ...
        return `[from:${record.content.from}@${fromVPort}|to:${record.content.to}@${toVPort}]`;
    }

    _onConnectBegin(record) {
        if (HIWIFI_ONLY && !Stat.isHiwifi(record.content.to)) {
            return;
        }

        let connectID = Stat.makeConnectID(record);
        let connectInfo = this.m_connections.get(connectID);
        let fromPeer = this.m_allPeers.get(record.content.from);
        let toPeer = this.m_allPeers.get(record.content.to);
    ...
    _onConnectSucc(record) {
        ...
        connectInfo.synConsum = record.content.detail.synConsum;
        let useTime = record.time - connectInfo.startClientTime;
        if (record.content.detail.useTCP) {
            this.m_tcpConnectCount++;
        }
        if (record.content.from === record.content.to) {
            this.m_selfSuccConnect++;
        }
        let peerInfo = this.m_allPeers.get(record.peerid);
        peerInfo.succTimes++;
        let toPeerInfo = this.m_allPeers.get(record.content.to);
        if (toPeerInfo) {
        ...
        // 统计各步骤消耗时长
        if (record.content.detail.sn.finishsn) {
        ...
        if (record.content.detail.sn.epTime || record.content.detail.sn.epTime === 0) {
    ...
    _onConnectError(record) {
        ...
        // call fail
        if (record.content.detail.sn.respEP1 && (!record.content.detail.sn.eplist || record.content.detail.sn.eplist.length == 0)) {
            this.m_failConnectionStat.callFail++;
        }

        if (!isBreak && toPeer && this._isPeerOnline(toPeer, record.serverTime) && record.content.detail.sn.snList && record.content.detail.sn.snList.length) {
            let onlineCount = 0;
        ...
        if (!isBothSymNAT && !acceptorOffline) {
            if (!isBreak) {
                // console.log(`connect failed, from:${record.content.from},to:${record.content.to}`);
            }
        }
        ...
        // dht timeout
        if (record.content.detail.dht.start && !record.content.detail.dht.finish1) {
        ...
        // dht fail
        if (record.content.detail.dht.start && record.content.detail.dht.finish1 && (!record.content.detail.dht.eplist || record.content.detail.dht.eplist.length === 0)) {
        ...
        // find sn fail
        if (record.content.detail.sn.start && (!record.content.detail.sn.snList || record.content.detail.sn.snList.length === 0)){
    ...
    _onConnectClose(record) {
        ...
        connectInfo.findSN = record.content.detail.sn.finishsn - record.content.detail.sn.start;
        connectInfo.snResp = record.content.detail.sn.respEP1 - record.content.detail.sn.finishsn;
        if (record.content.detail.sn.getEP || record.content.detail.sn.getEP === 0) {
        ...
        connectLog += `epcount:\t${record.content.detail.sn.eplist.length}\t`;
        connectLog += `eptime:\t${record.content.detail.sn.epTime}\t`;
        if (toPeer) {
        ...
        if (detail && detail.flow && connectInfo.succTime && record.time > connectInfo.succTime) {
            ...
            if (connectInfo.status === 'succ' && recvSendRate < 0.1) {
                console.log(`connection low recvSendRate: ${recvSendRate}, from:${record.content.from}@${record.content.fromVPort}, to:${record.content.to}@${record.content.vport}`);
            }
            */
        }
    ...
    _onConnectSuccAccept(record) {
        if (HIWIFI_ONLY && !Stat.isHiwifi(record.content.from)) {
            return;
        }
                ...
                rl.on('line', (line) => {
                    ...
                    if (record.type === 'connect-suc') {
                        ...
                        if (detail.ep && !EndPoint.isNAT(detail.ep) && !queryedRemoteEPList.has(detail.ep)) {
                            ...
                            let toSNInfo = this.m_allSNs.get(record.content.to);
                            if (toSNInfo) {
