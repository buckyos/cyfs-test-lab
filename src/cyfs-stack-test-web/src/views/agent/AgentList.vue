    ...
    <div class="table_container">
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <span style="padding-left: 10px;">{{ scope.row.owner}}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="device_id"
      ...
      <el-table-column
        ...
        width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.ood_runtime}}</span>
        </template>
      </el-table-column>
        <el-table-column
        label="系统类型"
      ...
      <el-table-column
        ...
        width="400">
        <template slot-scope="scope">
          <span  style="color:#67c23a">{{ scope.row.network }}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="stauts"
        ...
        width="100">
        <template slot-scope="scope">
          <span  style="color:#dd6161">{{ scope.row.status }}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="最新上线时间"
        ...
        <template slot-scope="scope">
          ...
          <span  style="color:#409eff">{{ scope.row.last_time }}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="备注"
       ...
       <el-table-column 
        ...
        <template slot-scope="scope">
          <el-button
            type="primary"
            size="small"
            @click="handlePerfView(scope.$index, scope.row)">查看</el-button>
          <el-button
            type="primary"
            size="small"
            @click="handlePerfStart(scope.$index, scope.row)">开启</el-button>
          <el-button
            size="small"
            type="danger"
            @click="handlePerfStop(scope.$index, scope.row)">暂停</el-button>
        </template>
...
export default {
  name: 'AgentList',
    ...
    methods:{
      getProfiles() {
        this.$axios.get('/api/agent',{params:{page_index:this.paginations.page_index,page_size:this.paginations.page_size}}).then(res => {
     ...
     handleDelete(index,row){  //删除数据
				...
				this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
					...
					.then(() => {
						this.$axios.delete(`/api/agent/delete/${row._id}`).then(res => {
							this.$message({
								message: '删除成功',
								type: 'success'
							})
