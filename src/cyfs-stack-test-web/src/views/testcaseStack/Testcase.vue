  ...
  <div class="testcase" style = "height:110%">
    <div>
      <el-form :inline="true" ref="search_data" :model="search_data">
        ...
        <el-form-item class="btnRight">
            <el-button type="primary" size ="small" icon="el-icon-edit-outline" @click='handleEdit()'>添加</el-button>
        </el-form-item>
        <el-form-item class="btnRight">
            <el-button type="primary" size ="small" icon="el-icon-edit-outline" @click='handleEdit()'>同步数据到本地</el-button>
        </el-form-item>
        <el-form-item class="btnRight">
            <el-button type="primary" size ="small" icon="el-icon-edit-outline" @click='handleEdit()'>上传数据到QAOOD</el-button>
        </el-form-item>
        <el-form-item class="btnRight">
            <el-button type="primary" size ="small" icon="el-icon-edit-outline" @click='handleEdit()'>获取数据从QAOOD</el-button>
        </el-form-item>
      </el-form>
    </div>
    <div class="table_container" >
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <i class="el-icon-time"></i>
          <span  style="color:#409eff">{{Number(scope.row.update_time) | moment }}</span>
        </template>
      </el-table-column>
...
export default {
  name: 'Testcase',
    ...
    methods:{
      async getProfiles() {
        this.$axios.get('/api/testcaseStack',{params:{page_index:this.paginations.page_index,page_size:this.paginations.page_size}}).then(res => {
     ...
     handleDelete(index,row){  //删除数据
            ...
            this.$message({
					...
					// 	this.$axios.delete(`/api/testcaseStack/delete/${row._id}`).then(res => {
					// 		this.$message({
					// 			message: '删除成功',
					// 			type: 'success'
					// 		})
      ...
      sendToOOD(){  //添加信息
        this.$axios.delete(`/api/testcaseStack/uploadOOD`).then(res => {
				this.$message({
						message: '上传成功',
						type: 'success'
					})
			...
			handleSearch() { //查询
				...
				this.$axios.post(`/api/testcaseStack/search`, this.search_data).then(res => {
            this.allTableData = res.data;
            this.filterTabDate = res.data; //过滤数据
            this.setPaginations();
            this.$message({
