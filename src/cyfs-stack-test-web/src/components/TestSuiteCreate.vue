  ...
  data() {
    return {
      ...
      formdialog: {
        ...
        TestSuiteId: [{ required: true, message: "自动化测试用例编号不能为空", trigger: "blur" }],
        TestSuite_name: [{ required: true, message: "测试检查要点不能为空", trigger: "blur" }]
      }
    };
  },
  methods: {
    dialongAdd(formdoalog) {
      this.$refs[formdoalog].validate(valid => {
        if (valid) {
          ...
          this.$axios.post(`/api/TestSuite/${url}`, this.formDate).then(res => {
            this.$message({
              message: "添加信息成功",
              type: "success"
            });
