...
export async function TaskMain(_interface: TaskClientInterface) {
    ...
    let testcase_name = "Connect_Close_UDP_RNUnlive"
    ...
    for(let i in LabAgent){
        for(let j in LabAgent ){
            if(i != j && LabAgent[i].NAT + LabAgent[j].NAT <5 ){
                ...
                // 1.4 RN 关闭连接
                info = await test_runner.prev_task_add_action(new BDTAction.DestoryAction({
                ...
                // 1.6 重新启动协议栈
                info = await test_runner.prev_task_add_action(new BDTAction.RestartAction({
