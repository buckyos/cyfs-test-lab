...
export async function TaskMain(_interface: TaskClientInterface) {
    ...
    let testcase_name = "NDN_BBR_TCP_File_Concurrent10_upload50"
    ...
    操作步骤：
        (1) RN track_file_in_path  5个40Mb大小文件，chunk 大小 10Mb， 10个User并行从RN下载文件;
    测试节点数据限制：
        (1) 所有机器组合
    预期结果：
    ...
    for(let index = 0;index<10;index++){
        Users.push(`${agent_list.LAN[index].tags[0]}$1`)
    }
