...
export async function TaskMain(_interface: TaskClientInterface) {
    let testcase_name = "Connect_IPV4_UDPTunnel_SN_invalidEP"
    ...
    let firstQA_answer= "";
    agentList = agentList.concat(await InitAgentData(testAgent,{ipv4:{udp:true}},"info",1,LabSnList,{},firstQA_answer,Resp_ep_type.default))
    for(let i in agentList){
        for(let j in agentList){
            if(i != j){
                // NAT穿透
                if(agentList[i].NAT + agentList[j].NAT < 5){
                    taskList.push(
                        {
