...
export async function TaskMain(_interface: TaskClientInterface) {
    let testcase_name = "Connect_IPV4_TCPTunnel_SN_TCPReverse"
    ...
    let firstQA_answer= "";
    agentList = agentList.concat(await InitAgentData(testAgent,{ipv4:{tcp:true}},"info",1,LabSnList,{tcpReserve:true},firstQA_answer,Resp_ep_type.Empty))
    for(let i in agentList){
        for(let j in agentList){
            if(i != j){
                // RN 是外网设备才能连接
                if(agentList[i].NAT == 0 && agentList[j].NAT != 0){
                    taskList.push(
                        {
