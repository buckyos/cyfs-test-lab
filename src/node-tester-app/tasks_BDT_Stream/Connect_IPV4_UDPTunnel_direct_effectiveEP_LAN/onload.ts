...
export async function TaskMain(_interface: TaskClientInterface) {
    let testcase_name = "Connect_IPV4_UDPTunnel_direct_effectiveEP_LAN"
    ...
    let firstQA_answer= "";
    agentList = agentList.concat(await InitAgentData(testAgent,{ipv4:{udp:true}},"info",1,[],{},firstQA_answer,Resp_ep_type.effectiveEP_LAN))
    for(let i in agentList){
        for(let j in agentList){
            if(i != j){
                // 同内网直连
                if(SameRouter(agentList[i].router!,agentList[j].router!)  ){
                    taskList.push(
                        {
