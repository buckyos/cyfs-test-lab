...
export async function TaskMain(_interface: TaskClientInterface) {
    let testcase_name = "Connect_IPV4_TCPTunnel_PortMap_SN"
    ...
    let firstQA_answer= "";
    agentList = agentList.concat(await InitAgentData(testAgent,{ipv4:{tcp:true,udp:true}},"info",1,LabSnList,{},firstQA_answer,Resp_ep_type.all))
    for(let i in agentList){
        for(let j in agentList){
            if(i != j){
                // LN 或 RN 开启端口映射
                if(testAgent[i].portMap || testAgent[j].portMap){
                    taskList.push(
                        {
