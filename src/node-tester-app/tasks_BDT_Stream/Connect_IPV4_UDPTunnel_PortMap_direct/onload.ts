...
export async function TaskMain(_interface: TaskClientInterface) {
    let testcase_name = "Connect_IPV4_UDPTunnel_PortMap_direct"
    ...
    let firstQA_answer= "";
    agentList = agentList.concat(await InitAgentData(testAgent,{ipv4:{udp:true}},"info",1,[],{},firstQA_answer))
    for(let i in agentList){
        for(let j in agentList){
            if(i != j){
                // RN 开启端口映射
                if(testAgent[j].portMap){
                    taskList.push(
                        {
