...
async function main() {
    ...
    var config : CaseConfig = require(`./${name}`);

    const serviceid = String(config.rust_bdt.serviceid);
    const taskList = config.rust_bdt.list
    for(let i in taskList){
        let runConfig = path.join(DirHelper.getLogDir(),"running.pid")
        if(!fs.existsSync(runConfig)){
            fs.createFileSync(runConfig)
        }
        let taskid =  String(taskList[i].desc!);
