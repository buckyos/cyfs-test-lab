...
async function main() {
    ...
    // await empyt_task();
    // await modify_import(part, name, '../../testcase_runner/cyfs_bdt_cli', '.');
    // await copy_tools('../testcase_runner/cyfs_bdt_cli', "../tasks"); //复制 testcase_runner
    // await start_zip(); //生成zip压缩文件
    const program = new Command('node_test');
    await program.addCommand(action.zip_case.makeCommand())
