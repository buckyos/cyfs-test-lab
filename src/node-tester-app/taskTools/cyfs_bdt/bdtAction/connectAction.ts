    ...
    async run(): Promise<{ err: number, log: string }> {
        ...
        if (info.answer) {
            if (info.answer != this.action.config.firstQA_answer) {
                this.logger!.error(`send answer = ${this.action.config.firstQA_answer} ,recv answer = ${info.answer}`);
        ...
        if (check.conn?.question!) {
            if (check.conn?.question != FirstQ) {
                this.logger!.error(`send question = ${FirstQ} ,recv question = ${check.conn?.question}`);
