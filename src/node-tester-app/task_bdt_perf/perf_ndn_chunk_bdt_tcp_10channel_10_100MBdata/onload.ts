...
export async function TaskMain(_interface: TaskClientInterface) {
    ...
    let testcase_name = "perf_ndn_chunk_bdt_tcp_10channel_10_100MBdata"
    let testcase: Testcase = {
        ...
        + LN RN 只使用BDT TCP连接 
        ## 操作步骤
        + (1) 10个LN 与 RN 之间并行 发送10 * 100MB chunk数据
        + (2) 维持连接2 min
        ## 性能监控
        + LN/RN 内存、CPU、网络带宽  `,
        environment: "lab",
    ...
    for (let i = 0; i < 10; i++) {
        ...
        for (let x = 0; x < 10; x++) {
            info = await test_runner.prev_task_add_action(new BDTAction.BdtTransChunkAction({
                LN: `${LN}$1$${i+1}`,
                RN: `${RN}$1$0`,
                config: {
                    conn_tag: connect_1,
