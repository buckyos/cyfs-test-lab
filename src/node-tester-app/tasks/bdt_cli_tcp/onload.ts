...
export async function TaskMain(_interface: TaskClientInterface) {
    ...
    let testcase_name = "perf_stream_connect_tcp_keep_100"
    ...
    await agent_manager.upload_system_info(testcase.testcase_id, 20000);
    //(4) 测试用例执行器添加测试任务
    for (let i = 0; i < 1; i++) {
        ...
        info = await test_runner.prev_task_add_action(new BDTAction.TcpCreateServerAction({
            LN: `${LN}$1$0`,
            config: {
                timeout: 20 * 1000,
                port : 22223,
