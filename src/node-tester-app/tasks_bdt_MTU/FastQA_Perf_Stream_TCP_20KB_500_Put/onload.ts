...
import {Testcase,Task,ActionType,Resp_ep_type,Listern_type} from "../../testcase_runner/cyfs_bdt/type"
...
export async function TaskMain(_interface: TaskClientInterface) {
    ...
    let testcase_name = "FastQA_Perf_Stream_TCP_20KB_500_Put"
    let testcase:Testcase = {
        testcase_name: testcase_name,
        testcase_id: `${testcase_name}_${Date.now()}`,
        remark: `测试性能:
        (1) LN RN 建立连接
        (2) LN->RN 使用Stream发送 20KB 请求数据
        (3) RN->LN 使用Stream回复 500 Bytes 请求数据`,
        environment: "lab",
    };
