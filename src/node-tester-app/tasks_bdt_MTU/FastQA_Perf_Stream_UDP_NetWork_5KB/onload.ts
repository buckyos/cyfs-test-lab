...
export async function TaskMain(_interface: TaskClientInterface) {
    ...
    let testcase_name = "FastQA_Perf_Stream_UDP_NetWork_5KB"
    let testcase:Testcase = {
        testcase_name: testcase_name,
        testcase_id: `${testcase_name}_${Date.now()}`,
        remark: `测试网络因素影响:
        (1)LN -> RN 发起连接请求 Question 带有 5*1024 Bytes请求数据 
        (2)RN 监听连接请求，收到连接请求后，解析Question,confrim 时回复5*1024Bytes响应数据`,
        environment: "lab",
    };
