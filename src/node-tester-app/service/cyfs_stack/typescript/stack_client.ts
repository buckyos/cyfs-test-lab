...
export class StackClient extends EventEmitter {
    ...
    async forward(obj_type: number, forward_params: WSParams.ForwardRequest): Promise<WSParams.ForwardResponse> {
        switch (forward_params.message_type) {
            case 'put_object':
                {
                    // 解码
                    const codec = new cyfs.NONPutObjectOutputRequestJsonCodec();
                    const req: cyfs.NONPutObjectOutputRequest = codec.decode_string(forward_params.req as string).unwrap();

                    let start_time = Date.now();
                    let put_resp = await this.m_stack!.non_service().put_object(req);
                    let opt_time = Date.now() - start_time;
                    if (put_resp.err) {
                        console.info(`${forward_params.message_type} failed, err = ${JSON.stringify(put_resp)}`)
                        return { err: ErrorCode.fail, log: `${JSON.stringify(put_resp)}` }
                    }
                    let data_size = req.object.object_raw.buffer.byteLength;

                    const resp_codec = new cyfs.NONPutObjectOutputResponseJsonCodec();
                    const data = JSON.stringify(resp_codec.encode_object(put_resp.unwrap()));

                    return {
                        err: ErrorCode.succ,
                        log: `${forward_params.message_type} success`,
                        opt_time,
                        data_size,
                        resp: data,
                    }
        ...
        return {
            ...
            log: `${forward_params.message_type} unknownCommand`
        }
    }
