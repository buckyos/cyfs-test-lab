...
export async function ServiceMain(_interface: ServiceClientInterface) {
    ...
    */
    ...
    _interface.registerApi('forward', async (from: Namespace, bytes: Buffer, param: { peerName: string, obj_type: number, forward_params: WSParams.ForwardRequest }): Promise<any> => {
        _interface.getLogger().debug(`remote call put_obejct`);
        // 
        let ret = await manager.forward(param.peerName, param.obj_type, param.forward_params);
        return { err: ret.err, bytes: Buffer.from(''), value: ret };
    });
