...
class ProxyManager extends EventEmitter {
    ...
    private socketList : Array<{socket:net.Socket,type:string,remoteAddress:string,remotePort:number,seq?:number,r_seq?:number}>; //本地socket 代理池 seq : SDK 到 协议栈序列号 , r_seq 协议栈到SDK序列号
    ...
    async proxy_data(type:string,remoteAddress:string,remotePort:number,seq:number,bytes: Buffer){
        for(let i in this.socketList){
            if(this.socketList[i].type == type && this.socketList[i].remoteAddress == remoteAddress && this.socketList[i].remotePort == remotePort){
                ...
                // while(recheck>0){
                //     recheck = recheck -1;
                //     if(this.socketList[i].seq == seq -1){
                //         this.socketList[i].seq = seq -1
                ...
                // }
                // if(recheck<=0){
                //     this.log.info(` ${this.peerName} TCP Client ${remotePort} write msg err, local req =${this.socketList[i].seq},recv_req=${seq}`);
                // }
            }
        }
