...
import {dingdingConfig,dingdingConfig_test} from "./config";
...
export async function dingding_test(content:string){
    let dingdingUrl = dingdingConfig_test.NFT.dingdingUrl
    let  secret=dingdingConfig_test.NFT.secret
    let dinfdingMsg =  {
        "at": dingdingConfig_test.NFT.msg.at,
        "text": {
            "content": "测试"
        },
        "msgtype":dingdingConfig_test.NFT.msg.msgtype
    };
    dinfdingMsg.text.content = content;
    let postData= JSON.stringify(dinfdingMsg);
    let header = {'Content-Type': 'application/json','Content-Length': Buffer.byteLength(postData)}
