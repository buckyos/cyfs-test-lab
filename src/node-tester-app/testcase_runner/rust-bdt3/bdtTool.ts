...
// StreamGuard {stream:StreamContainer {sequence:TempSeq(3783866420), local:5aSixgLrH4Z6U7ACTXH4PZ7YAgiYL7JhGuvoak9jCmBh, remote:5aSixgLwKxx3S97pXfpPr7xcpdfwFNVoRDoCMZGsLBEr, port:0, id:1297450350 }}
...
export class BdtPeer extends EventEmitter{
    ...
    on(event: 'accept', listener: (RNconn:BdtConnection,peerName?:String) => void):this;
    ...
    once(event: 'accept', listener: (RNconn:BdtConnection,peerName?:String) => void):this;
    ...
    getConnect(name:string,remote:string):{err:boolean,conn?:BdtConnection}{
    ...
    async containConn(name:string,remote:string):Promise<{err:boolean,conn?:BdtConnection}>{
        for(let i =0;i<10;i++){
            let info = this.getConnect(name,remote)
            if(!info.err){
                return info;
            }
            await sleep(500);
    ...
    async start(addrInfo: string[], local: string, snFiles: string[], knownPeer?: Buffer[],RUST_LOG?:string,activePnFiles?:Array<string>, passivePnFiles?:Array<string>,knownPeerFiles?:Array<string>,chunk_cache?:string): Promise<ErrorCode> {
        let param: any = {
            addrInfo,
            snFiles,
            local,
    ...
    async connect(peer: BdtPeer, question: string, known_eps: number, remote_sn?: string): Promise<{err: ErrorCode, time?: number, conn?: BdtConnection}> {
        let param = {
            peerName: this.m_peerName,
            question,
            known_eps,
        ...
        return {err: ErrorCode.succ, time: info.value.time, conn};
    }
    ...
    async autoAccept(): Promise<ErrorCode> {
        ...
        if(this.m_acceptCookie==undefined){
            ...
            let rnAccept = await this.m_interface.attachEvent('accept', (err: ErrorCode,namespace: Namespace, connName: string,peerName:string) => {
                this.m_interface.getLogger().info(`${this.tags} ${peerName} 触发 accept conn = ${connName}`);
                if(this.peername === peerName){
                    let RNconn = new BdtConnection({agentid:this.m_agentid,peerName:peerName,connName,_interface:this.m_interface ,timeout:this.m_timeout})
                    this.emit('accept', RNconn,peerName);
                }
            }, this.m_agentid, this.m_timeout);
