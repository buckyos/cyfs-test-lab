...
export class BdtPeerClient extends EventEmitter {
    ...
    async connectList(remote_desc_list: Array<{ device_path: string }>, question: string, known_eps: number, accept_answer: number, conn_tag: string, remote_sn?: string,): Promise<{ err: ErrorCode, records?: string }> {
        ...
        this.logger.debug(`callApi connect-list BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
    ...
    async calculateChunk(path: string, chunk_size: number): Promise<{ err: ErrorCode, calculate_time?: number, chunk_id?: string }> {
        ...
        this.logger.debug(`callApi calculate-chunk BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
    ...
    async setChunk(path: string, chunk_id: string, chunk_size: number): Promise<{ err: ErrorCode, set_time?: number, chunk_id?: string }> {
        ...
        this.logger.debug(`callApi set-chunk BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
    ...
    async trackChunk(path: string, chunk_size: number): Promise<{ err: ErrorCode, set_time?: number, calculate_time?: number, chunk_id?: string }> {
        ...
        this.logger.debug(`callApi track-chunk BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
    ...
    async interestChunk(remote: Buffer, chunk_id: string): Promise<{ err: ErrorCode }> {
        ...
        this.logger.debug(`callApi interest-chunk BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
    ...
    async checkChunk(chunk_id: string): Promise<{ err: ErrorCode, state?: string }> {
        ...
        this.logger.debug(`callApi check-chunk BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
    ...
    async interestChunkList(remote: Buffer, task_name: string, chunk_list: Array<{ chunk_id: string }>): Promise<{ err: ErrorCode, session?: string }> {
        ...
        this.logger.debug(`callApi interest-chunk-list BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
    ...
    async checkChunkList(session: string): Promise<{ err: ErrorCode, state?: string }> {
        ...
        this.logger.debug(`callApi check-chunk-list BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
    ...
    async sendFile(path: string, chunk_size: number): Promise<{ err: ErrorCode, calculate_time?: number, set_time?: number, file?: Buffer, ObjectId?: string }> {
        ...
        this.logger.debug(`callApi start-send-file BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
    ...
    async calculateFile(path: string, chunk_size: number): Promise<{ err: ErrorCode, calculate_time?: number, file?: Buffer, ObjectId?: string }> {
        ...
        this.logger.debug(`callApi calculate-file BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
    ...
    async setFile(path: string, file: Buffer): Promise<{ err: ErrorCode, set_time?: number, file?: Buffer, ObjectId?: string }> {
        ...
        this.logger.debug(`callApi set-file BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
    ...
    async downloadFile(file: Buffer, save_path: string, peer_id: string, second_peer_id?: string): Promise<{ err: ErrorCode, file?: Buffer, session?: string }> {
        ...
        this.logger.debug(`callApi start-download-file BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
    ...
    async downloadFileRange(file: Buffer, save_path: string, peer_id: string, ranges: Array<{ begin: number, end: number }>, second_peer_id?: string): Promise<{ err: ErrorCode, file?: Buffer, session?: string }> {
        ...
        this.logger.debug(`callApi start-download-file-range BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
            this.logger.error(`${this.tags} downloadFile failed,err =${info.err} ,info =${JSON.stringify(info.value)}`)
        }
        return { err: info.err, file: info.bytes, session: info.value?.session };
    ...
    async downloadTaskState(session: string): Promise<{ err: ErrorCode, state?: string }> {
        ...
        this.logger.debug(`callApi download-file-state BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
    ...
    async setDir(path: string, object_path: string, chunk_size: number): Promise<{ err: ErrorCode, dir_object_path?: string, session?: string, dir_id?: string, dir_map?: Array<{ name: string, file_id: string }> }> {
        ...
        this.logger.debug(`callApi start-send-dir BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err || info.value.result) {
            this.logger.error(`${this.tags} setFile failed,err =${info.err} ,info =${JSON.stringify(info.value)}`)
    ...
    async downloadDir(save_path: string, dir_name: string, peer_id: string, object_path: string, dir_id: string, dir_map: Array<{ name: string, file_id: string }>, second_peer_id?: string): Promise<{ err: ErrorCode, file?: Buffer, session?: string }> {
        ...
        this.logger.debug(`callApi start-download-dir BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
            this.logger.error(`${this.tags} downloadFile failed,err =${info.err} ,info =${JSON.stringify(info.value)}`)
        }
        return { err: info.err, file: info.bytes, session: info.value?.session };
    ...
    async downloadDirTaskState(session: string): Promise<{ err: ErrorCode, state?: string }> {
        ...
        this.logger.debug(`callApi download-dir-state BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err) {
    ...
    async connect_send_stream(conn_tag:string,remote: Buffer, question_size: number, known_eps: number): Promise<{resp:api.ConnectSendStreamResp,conn?:BdtConnection}> {
        let info = await this.m_interface.callApi('sendBdtLpcCommand', remote, {
            name: 'connect_send_stream',
            peerName: this.peerName,
            unique_id: this.peerName,
            question_size,
        }, this.m_agentid, 0);
        this.logger.debug(`callApi connect_send_stream BuckyResult result = ${info.value.result},msg = ${info.value.msg}`)
        if (info.err || info.value.result) {
            this.logger.error(`${this.tags}  connect_send_stream failed,err =${info.err} ,info =${JSON.stringify(info.value)}`)
    ...
    async auto_response_stream(answer_size?: number): Promise<api.ListenerStreamLpcCommandResp> {
        ...
        if (info.err) {
            this.logger.error(`${this.tags} auto_response_stream failed,err =${info.err} ,info =${JSON.stringify(info.value)}`)
        }
        //监听accept 
        if (this.m_acceptCookie == undefined) {
            let rnAccept = await this.m_interface.attachEvent(`auto_response_stream_${this.peerid}`, (err: ErrorCode, namespace: Namespace, json: string) => {
                let eventIfo: api.ListenerStreamEventLpcCommandResp = JSON.parse(json);
                if (eventIfo.result == 0) {
                    ...
                    let fastQAInfo: FastQAInfo = {
                        ...
                        file_szie : answer_size,
                        send_time : eventIfo.send_time,
                        recv_time : eventIfo.recv_time,
                        send_hash: eventIfo.send_hash,
                        recv_hash: eventIfo.recv_hash,
                    }
                    let conn = new BdtConnection({ agentid: this.m_agentid, peerName: this.peerName!, stream_name: eventIfo.stream_name, _interface: this.m_interface, timeout: this.m_timeout,fastQAInfo, conn_tag: this.conn_tag})
    ...
    async connect(remote: Buffer, question: number, known_eps: number, accept_answer: number, conn_tag: string, remote_sn?: string,): Promise<{resp:api.ConnectLpcCommandResp,conn?:BdtConnection}> {
        let info = await this.m_interface.callApi('sendBdtLpcCommand', remote, {
            name: 'connect',
            peerName: this.peerName,
            unique_id: this.peerName,
    ...
    async autoAccept(answer_size?:number): Promise<api.AutoAcceptStreamLpcCommandResp> {
        let info = await this.m_interface.callApi('createBdtLpcListener', Buffer.from(""), {
            name: 'auto_accept',
            peerName: this.peerName,
            unique_id: this.peerName,
