...
export class TestRunner{
    ...
    private agent_manager : AgentManager; // 测试设备列表
    ...
    constructor(_interface:TaskClientInterface,is_perf:boolean=false){
        ...
        this.agent_manager =  AgentManager.create_instance(_interface); 
        this.errorList = [];
    ...
    async init_testcase(testcase:Testcase){
        ...
        if(fs.existsSync(path.join(__dirname,"../dev_config.js"))){
            ...
            this.Testcase.environment = testcase.testcase_id; 
        }
        
    }

    create_prev_task(task:Task):{err:number,log:string}{
        if(this.cacheTask){
            this.logger.error(`create_prev_task failed,task already exsit`)
            return {err:BDTERROR.optExpectError,log:"create_prev_task failed,task already exsit"}
        }
        this.cacheTask = task; 
        return {err:BDTERROR.success,log:"createPrevTasksuccess"}
    }
    prev_task_add_action(action:ActionAbstract):{err:number,log:string}{
        if(!this.cacheTask){
            this.logger.error(`prev_task_add_action failed,task not exsit`)
            return {err:BDTERROR.optExpectError,log:"prev_task_add_action failed,task not exsit"}
        }
        this.cacheTask!.action.push(action); 
        return {err:BDTERROR.success,log:"prev_task_add_action add success"}
    }
    prev_task_run():{err:number,log:string}{
        if(!this.cacheTask){
            this.logger.error(`prev_task_run failed,task not exsit`)
            return {err:BDTERROR.optExpectError,log:"prev_task_run failed,task not exsit"}
        }
        ...
        return {err:BDTERROR.success,log:"prev_task_run begin run"}
    }
    // 运行测试任务
    async runTask(task:Task):Promise<{err:number,log:string, record : {taskId:string,data:Array<any>}}>{
        // 判断机器状态是否正常，机器不正常
        return new Promise(async(V)=>{
            let check = await this.agent_manager.checkBdtPeerClientList(task.LN,task.RN,task.Users);
            task.state = "run" ;
            if(!task.timeout){
    ...
    async addTask(task:Task):Promise<{err:number,log:string}>{
        this.total++
        task.testcase_id = this.Testcase!.testcase_id!;
        task.task_id = `${this.Testcase!.testcase_id!}_${RandomGenerator.string(10)}`;
    ...
    async saveTestcase() {
        ...
        this.JSONReport.TestcaseInfo = {
            ...
            agentList: JSON.stringify(this.agent_manager.agentListState),
    ...
    async exitTestcase(err:number,log:string){
        ...
        setTimeout(async()=>{
            await this.agent_manager.stopService();
        ...
        //打印错误数据
        this.logger.info(`######## Tescase run finished ,testcase_id = ${this.Testcase!.testcase_id}`)
    ...
    async wait_finished(check:number = 5){
        while(true){
