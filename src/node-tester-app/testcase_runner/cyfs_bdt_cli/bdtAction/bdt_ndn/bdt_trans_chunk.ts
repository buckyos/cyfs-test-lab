...
export class BdtTransChunkAction extends BaseAction implements ActionAbstract {
    async run(): Promise<{ err: number, log: string }> {
        ...
        // (2) LN 生成随机文件
        let create_file = await (await this.agent_manager!.get_bdt_peer_client(this.action.LN)).client!.util_client!.createFile(this.action.fileSize!);
        let rn_cache_path = (await this.agent_manager!.get_bdt_peer_client(this.action.RN!)).client!.util_client!.cachePath;
        // (3) LN 上传文件chunk到 BDT
        ...
        let begin  = Date.now();
        let create_task_result = await RN.bdt_stack!.interest_chunk(LN.bdt_stack!.device_object,publish_result.resp!.chunk_id,rn_cache_path?.file_download);
