...
export class BdtCli extends EventEmitter {
    ...
    async init(index: number = 1): Promise<{ err: number, log?: string }> {
        ...
        return new Promise(async (resolve) => {
            ...
            if (start_tool.err || start_tool.value.result) {
                this.logger.error(`${this.tags} start bdt-tools failed  result = ${start_tool.value.result},msg = ${start_tool.value.msg}ji`)
