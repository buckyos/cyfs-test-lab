...
export class TestRunner{
    ...
    async saveTask(){
        ...
        for(let i in this.Testcase!.taskList){
            ...
            runList.push(new Promise(async(V)=>{
                ...
                this.log.info(`api/bdt/action/addList req: ${this.Testcase!.testcase_id}_task${i}`)
    ...
    async start_report(testcase_id:string){
        for(let i in this.agentList!){
    ...
    async exitTestcase(result:number,log:string){
        ...
        for(let i in this.Testcase!.agentList!){
            let info = await this.m_interface.callApi('reportLog', Buffer.from(''), {logName:`${this.Testcase!.testcase_name}_${this.Testcase!.agentList![i].name}.zip`},this.Testcase!.agentList![i].agentid!,timeout )
            this.Testcase!.agentList![i].logUrl = info.value.url;
            this.log.info(`**** ${this.Testcase!.testcase_name} ${this.Testcase!.agentList![i].logUrl}`)
        }
    ...
    async testCaseRunner(testcase:Testcase){
        ...
        await this.start_report(testcase.testcase_id);
