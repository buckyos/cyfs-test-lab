...
export class TestRunner extends EventEmitter {
    ...
    async connectAgent() {
        ...
        for (let i in this.Testcase!.agentList) {
            promiseList.push(new Promise(async (V) => {
                ...
                //FIXME: 只用了agent_id 机器mac地址,  (agent_id  net_info代理机器ip, port信息!!!)
    ...
    async saveTestcaseToMysql() {
        //(1)保存testcase
        this.log.info(`api/cyfs/cyfs_testcase/add req: ${JSON.stringify(this.Testcase!.testcase_id)}`)
    ...
    async exitTestcase(result: number, log: string) {
        ...
        if (this.state != "end") {
            ...
            if (result) {
                ...
                this.log.info(`##测试用例执行完成，${this.Testcase!.testcase_id}`)
    ...
    async put_obeject(task: number, action: number): Promise<{ err: number, log: string }> {
        ...
        let put_object_params: WSParams.PutObjectParmas = {
            obj_type: this.Testcase!.taskList[task].action[action].input_data.obj_type,
            common: {
                level: this.Testcase!.taskList[task].action[action].input_data.common.level,
                target: target.peer!.deviceId!,
                flags: this.Testcase!.taskList[task].action[action].input_data.common.flags,
            }
        }
        let put_resp = await source.peer!.put_obejct(obj_type, put_object_params);
        if (put_resp.err) {
            return { err: put_resp.err, log: put_resp.log! };
        }
        this.Testcase!.taskList[task].action[action].opt_time = put_resp.opt_time;
        this.Testcase!.taskList[task].action[action].data_size = put_resp.data_size;
        return { err: StackError.success, log: `get_object ${put_resp.object_id} success` };
    }
    async put_object_check(task: number, action: number): Promise<{ err: number, log: string }> {
        let source = this.m_cyfsProxy.getPeer(this.Testcase!.taskList[task].action[action].source)
        let target = this.m_cyfsProxy.getPeer(this.Testcase!.taskList[task].action[action].target!)
        if (source.err) {
            return { err: StackError.connect_cyfs_client_faild, log: `${this.Testcase!.taskList[task].action[action].source} getPeer error` };
        ...
        this.Testcase!.taskList[task].action[action].opt_time = put_resp.opt_time;
        this.Testcase!.taskList[task].action[action].data_size = put_resp.data_size;
        let get_resp = await target.peer!.get_obejct(obj_type, {
            obj_id: put_resp.object_id!,
            common: {
                level: "noc",
                flags: 0,
            }
        ...
        if (get_resp.err) {
            return { err: get_resp.err, log: get_resp.log! };
        }
        // get object 操作可以放到子操作
        this.Testcase!.taskList[task].child_action!.push({
            action_id: `${this.Testcase!.taskList[task].action[action].action_id}_get_obejct`,
            parent_action: this.Testcase!.taskList[task].action[action].action_id,
            type: "get_object",
            source: this.Testcase!.taskList[task].action[action].target!,
            opt_time: get_resp.opt_time,
            data_size: get_resp.data_size,
            input_data: {
                obj_type: WSParams.CustumObjectType.MyText,
                common: {
                    level: "noc",
                    flags: 0
                }
        ...
        return { err: StackError.success, log: `get_object_check ${put_resp.object_id} success` };
    }


    async testCaseRunner(testcase: Testcase) {
        // FIXME: 连接代理, 打开协议栈初始化信息, 代理server端支持多路复用分流
        //(1) 连接测试框架节点
        this.state = "run";
        ...
        for (let i in this.Testcase.taskList) {
            ...
            taskRun.push(new Promise<{ err: number, log: string }>(async (V) => {
                ...
                for (let j in this.Testcase!.taskList[i].action) {
                    this.Testcase!.taskList[i].action[j].action_id = `${this.Testcase!.testcase_id}_task${i}_action${j}`;
                    if (error_break) {
                        break;
                    }
                    ...
                    switch (this.Testcase!.taskList[i].action[j].type) {
                        case "put_obejct": {
                            result = await this.put_obeject(Number(i), Number(j));
                            break;
                        }
                        case "put_object_check": {
                            result = await this.put_object_check(Number(i), Number(j));
                            break;
                        }
                    }
