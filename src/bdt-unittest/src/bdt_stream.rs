...
impl BDTConnection {
    ...
    pub async fn send_data(&mut self, size: u64) -> Result<HashValue, BuckyError> {
        ...
        loop {
            ...
            hashs.push(hash);
            log::debug!("send data len = {}",gen_count);
    ...
    pub async fn recv_data(&mut self) -> Result<(u64, HashValue), BuckyError> {
        log::info!("start recv data");
        let mut hashs = Vec::<HashValue>::new();
