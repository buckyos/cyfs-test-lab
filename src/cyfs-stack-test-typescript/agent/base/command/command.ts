...
function encodeNamespace(namespace: Namespace, writer: BufferWriter): ErrorCode {
    if (namespace.agentid) {
        writer.writeU16(Buffer.byteLength(namespace.agentid!, 'utf-8'));
        writer.writeString(namespace.agentid, 'utf-8')
    } else {
        writer.writeU16(0);
    }

    if (namespace.serviceid) {
        writer.writeU16(Buffer.byteLength(namespace.serviceid!, 'utf-8'));
        writer.writeString(namespace.serviceid, 'utf-8')
    } else {
        writer.writeU16(0);
    }

    if (namespace.taskid) {
        writer.writeU16(Buffer.byteLength(namespace.taskid!, 'utf-8'));
        writer.writeString(namespace.taskid, 'utf-8')
    } else {
        writer.writeU16(0);
    }
...
export function encodeCommand(c: Command, writer: BufferWriter): ErrorCode {
    ...
    writer.writeU16(Buffer.byteLength(c.name, 'utf-8'));
    writer.writeString(c.name, 'utf-8');
