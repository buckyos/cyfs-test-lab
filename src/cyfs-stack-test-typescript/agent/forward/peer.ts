...
export class StackPeer extends EventEmitter {
    ...
    async forward(obj_type: number, forward_params: WSParams.ForwardRequest): Promise<WSParams.ForwardResponse> {
        ...
        let info = await this.m_interface.callApi('forward', Buffer.from(''), param, this.m_agentid!, 0);
        if (info.err) {
            this.m_interface.getLogger().error(`callApi forward failed ,err = ${JSON.stringify(info)} `)
            return { err: info.err, log: info.value.log }
        }
        return info.value;
    }
