...
let stack_device:cyfs.SharedCyfsStack;
...
describe("SharedCyfsStack util相关接口测试",function(){
    ...
    it("create_task 接口调用",async()=>{
        ...
        let task1 = await stack_runtime.trans().get_task_state( {
            common:  {
                req_path: "qaTest",
                dec_id: ZoneSimulator.APPID,
                level: cyfs.NDNAPILevel.NDN,
        ...
        console.info("create_task 接口调用",JSON.stringify(task1));
        assert.ok(!task1.err,"control_task 失败");
    })
    it("stop_task 接口调用",async()=>{
        ...
        console.info("stop_task 接口调用",JSON.stringify(task1));
        assert.ok(!task1.err,"stop_task 失败");
    })
    it("start_task 接口调用",async()=>{
        ...
        console.info("start_task 接口调用",JSON.stringify(task1));
        assert.ok(!task1.err,"start_task 失败");
    })
    it("control_task 接口调用",async()=>{
        ...
        console.info("control_task stop接口调用",JSON.stringify(task));
    ...
    it("control_task 接口调用",async()=>{
        ...
        console.info("control_task start 接口调用",JSON.stringify(task));
        assert.ok(!task.err,"control_task 失败");
    })
    ...
    it("下载进度测试",async()=>{
        ...
        stack_runtime = ZoneSimulator.zone1_device1_stack!;
        stack_ood = ZoneSimulator.zone1_ood_stack!;
        stack_device = ZoneSimulator.zone1_device2_stack!;
        let fileName = RandomGenerator.string(10);
        ...
        console.info("add_file",JSON.stringify(add_file));
        assert.ok(!add_file.err,"publish_file 失败");
        test_file = add_file.unwrap().file_id;
        // let task = await stack_runtime.trans().create_task( {
        //     common:  {
        ...
        let stream = await stack_runtime.non_service().put_object({
            common: {
                dec_id:ZoneSimulator.APPID,
                flags: 0,
                target: stack_device.local_device_id().object_id,
        ...
        const get_ret2 = await stack_device.non_service().get_object(req1);
        let save_filePath = path.join(__dirname,"../../test_cache_file/target")
        console.info(`get_ret2 ,${JSON.stringify(get_ret2) }`)
        let download = await stack_device.trans().create_task( {
            common:  {
                req_path: "qaTest",
                dec_id: ZoneSimulator.APPID,
