...
async fn main() {
    let app = App::new("zone-simulator")
        ...
        .arg(Arg::with_name("user_num").long("user_num").takes_value(true).help("user_num").default_value("2"))
        .arg(Arg::with_name("user_num").long("user_num").takes_value(true).help("user_num").default_value("2"))
        .arg(Arg::with_name("bdt_port").long("bdt_port").takes_value(true).help("default begin bdt_port").default_value("20000"))
        .arg(Arg::with_name("service_port").long("service_port").takes_value(true).help("default begin service_port").default_value("21000"));
    ...
    let users = TestLoader::load_users(profile::TEST_PROFILE.get_mnemonic(), true, dump,user_num).await;
