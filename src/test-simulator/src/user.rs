...
impl TestUser {
    ...
    pub async fn load_user_from_local(name: &str, mnemonic: &str,user_path:&Path) -> Self{
        // 加载people desc
        let people_desc = &user_path.join("people.desc");
        let mut file = async_std::fs::File::open(people_desc).await.map_err(|e| {
            log::error!("open  desc failed , path={:?}, e={}", people_desc.display(), &e);
        }).unwrap();
        let mut buf = Vec::<u8>::new();
        let _ = file.read_to_end(&mut buf);
        let (people, _) = People::raw_decode(buf.as_slice()).unwrap();

        // 加载people sec
        let people_sec = &user_path.join("people.sec");
        let mut file = async_std::fs::File::open(people_desc).await.map_err(|e| {
            log::error!("open  desc failed , path={:?}, e={}", people_desc.display(), &e);
        }).unwrap();
        let mut buf = Vec::<u8>::new();
        let _ = file.read_to_end(&mut buf);
        let (sk, _) = PrivateKey::raw_decode(buf.as_slice()).unwrap();

        // 加载 ood desc
        let ood_desc = &user_path.join("ood.desc");
        let mut file = async_std::fs::File::open(ood_desc).await.map_err(|e| {
            log::error!("open  desc failed , path={:?}, e={}", ood_desc.display(), &e);
        }).unwrap();
        let mut buf = Vec::<u8>::new();
        let _ = file.read_to_end(&mut buf);
        let (ood, _) = Device::raw_decode(buf.as_slice()).unwrap();

        // 加载 ood sec
        let ood_sec = &user_path.join("ood.sec");
        let mut file = async_std::fs::File::open(ood_sec).await.map_err(|e| {
            log::error!("open  desc failed , path={:?}, e={}", ood_sec.display(), &e);
        }).unwrap();
        let mut buf = Vec::<u8>::new();
        let _ = file.read_to_end(&mut buf);
        let (ood_sk, _) = PrivateKey::raw_decode(buf.as_slice()).unwrap();

        let ood_work_mode = people.ood_work_mode();
