...
async function main() {
    ...
    //(3) 参数边界值之类的东西
    let buf = stringToUint8Array(RandomGenerator.string(6*1024*1024)) //单个值
    ...
    // list 一些结构
    let deviceList : Array<cyfs.ObjectId> = []
    for(let i =0 ; i<10000;i++){
        ...
        let category = cyfs.DeviceCategory.OOD;
        let device = cyfs.Device.create(cyfs.Some(ower.calculate_id()),unique_id,[],[],[],public_key_device,area,category)
        deviceList.push(device.calculate_id());
    }
    let people2 =  cyfs.People.create(cyfs.Some(ower.calculate_id()), deviceList, public_key, cyfs.None);
}
