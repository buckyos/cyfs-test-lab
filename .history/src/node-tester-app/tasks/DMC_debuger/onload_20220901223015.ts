...
async function test_file(_interface: TaskClientInterface,DMC_Download:StackProxyClient,DMC_Upload:StackProxyClient,stack_download:cyfs.SharedCyfsStack,stack_upload:cyfs.SharedCyfsStack) {
    ...
    let createFile = await DMC_Upload.util_client!.createFile(100*1024*1024);
    ...
    for(let i =0;i<10;i++){
        ...
        _interface.getLogger().info(JSON.stringify(task));
