...
export async function  download(_interface: TaskClientInterface,DMC_Download:StackProxyClient,stack_download:cyfs.SharedCyfsStack,stack_upload:cyfs.SharedCyfsStack,fileSize:number,timeout:number,test_file:cyfs.ObjectId,fileName:string){
    let dec_id = cyfs.ObjectId.from_base_58("9tGpLNnab9uVtjeaK4bM59QKSkLEGWow1pJq6hjjK9MM").unwrap();
    ...
    for(let i =0;i<timeout;i++){
        ...
        if(state==5){
            ...
            fs.appendFileSync(errorPath,`fileId=${test_file.to_base_58()}#fileName=${fileName}`)
...
export async function TaskMain(_interface: TaskClientInterface) {
    ...
    if(fs.pathExistsSync(errorPath)){
        ...
        let fileId = cyfs.ObjectId.from_base_58(data).unwrap();
        let result = await  download(_interface,DMC_Download,stack_download,stack_upload,fileSize,400,test_file,createFile.fileName!)
        await _interface.exit(ClientExitCode.failed, `${result}`)
    }
    let result = await test_file(_interface,DMC_Download,DMC_Upload,stack_download,stack_upload,50*1024*1024,400);
