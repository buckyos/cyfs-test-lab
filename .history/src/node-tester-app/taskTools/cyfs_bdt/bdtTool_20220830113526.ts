...
export class BdtPeerClient extends EventEmitter{
    ...
    async calculateFile(path:string,chunk_size:number): Promise<{err: ErrorCode, calculate_time?: number, file?: Buffer}>{
        let info = await this.m_interface.callApi('sendBdtLpcCommand',  Buffer.from(""), {
            name: 'calculate-file',
            peerName: this.peerName,
            chunk_size,
        ...
        return {err: info.err,calculate_time: info.value?.calculate_time,file:  info.bytes};
    }
    async on_set_file(path:string,file:number): Promise<{err: ErrorCode, calculate_time?: number, file?: Buffer}>{
        let info = await this.m_interface.callApi('sendBdtLpcCommand',  Buffer.from(""), {
            name: 'calculate-file',
            peerName: this.peerName,
            file,
        ...
        return {err: info.err,calculate_time: info.value?.calculate_time,fileObj:  info.bytes};
    }
