...
export class BdtPeerClient extends EventEmitter{
    ...
    async checkChunkListener(chunk_id:string,interval:number,timeout:number): Promise<{err: ErrorCode,state?:string,time?:number,record?:Array<{time:number,speed:string,progress:string}>}>{
        let begin = Date.now();
        let record:Array<{time:number,speed:string,progress:string}> = [{time:Date.now(),speed:"0",progress:"0"}];
        while(Date.now() - timeout < begin){
            let check = await this.downloadTaskState(chunk_id);
