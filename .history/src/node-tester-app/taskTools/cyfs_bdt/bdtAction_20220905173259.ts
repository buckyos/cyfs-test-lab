...
export class ActionBase implements ActionAbstract{
    ...
    async init(_interface: TaskClientInterface, task?: Task,index?:number): Promise<{ err: number, log: string }> {
        ...
        this.action.action_id = `${task?.task_id}_action${index}_${this.action.type}`
...
export class ConnectAction extends ActionBase implements ActionAbstract {
    async run(): Promise<{ err: number, log: string }> {
        ...
        //校验RN 连接成功
        let check = await RN.bdtClient!.remark_accpet_conn_name(info.conn!.TempSeq, LN!.bdtClient!.peerid!,info.conn!.id, this.action.config!.conn_tag);
        if (check.err) {
            // 等待2s再试一次，监听事件可能有网络延时
            await sleep(2000);
            check = await await RN.bdtClient!.remark_accpet_conn_name(info.conn!.TempSeq, LN!.bdtClient!.peerid!,info.conn!.id, this.action.config!.conn_tag);
            if (check.err) {
