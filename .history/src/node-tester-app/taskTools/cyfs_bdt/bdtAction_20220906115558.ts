...
export class ActionBase implements ActionAbstract{
    ...
    async save(): Promise<{ err: number, log: string }> {
        let run_action =await request("POST","api/bdt/action/add",{
            ...
            type: this.action.child_action![j]!.type,
            action_id:this.action.child_action![j].action_id,
            parent_action :this.action.child_action![j].parent_action,
            LN:this.action.child_action![j].LN.name,
            RN:this.action.child_action![j].RN?.name,
            Users: JSON.stringify(UserName),
            config:JSON.stringify( this.action.child_action![j].config),
            info:JSON.stringify( this.action.child_action![j].info),
            fileSize : this.action.child_action![j].fileSize,
            chunkSize : this.action.child_action![j].chunkSize,
            connect_time : this.action.child_action![j].connect_time,
            set_time : this.action.child_action![j].set_time,
            send_time : this.action.child_action![j].send_time,
            result: String(this.action.action[j].result!.err),
            expect:String(this.action.action[j].expect!.err),
        },ContentType.json);
