...
export class BdtPeerClient extends EventEmitter{
    ...
    async getCachePath(filePath:string):Promise<{err:ErrorCode,cache_path? : {file_upload:string,file_download:string,NamedObject:string}}>{
        let result = await this.m_interface.callApi('utilRequest', Buffer.from(''), {
            name : "md5",
            peerName: this.peerName,
            filePath,
        ...
        return {err:ErrorCode.exception,cache_path:result.value.md5}
    }
    ...
    async downloadTaskListener(session:string,interval:number,timeout:number): Promise<{err: ErrorCode,state?:string,record?:Array<{time:number,speed:string,progress:string}>}>{
        let begin = Date.now();
        ...
        while(Date.now() - timeout < begin){
            ...
            if( !check.state || !check.state.includes("Downloading")){
                return {err:check.err,state:check.state,record}
            }
            record.push({
                time: Date.now(),
