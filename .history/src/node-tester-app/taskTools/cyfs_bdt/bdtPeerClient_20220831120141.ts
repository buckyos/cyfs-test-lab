...
export class BdtPeerClient extends EventEmitter{
    ...
    async checkChunkListener(session:string,interval:number,timeout:number): Promise<{err: ErrorCode,state?:string,time?:number,record?:Array<{time:number,speed:string,progress:string}>}>{
        let begin = Date.now();
