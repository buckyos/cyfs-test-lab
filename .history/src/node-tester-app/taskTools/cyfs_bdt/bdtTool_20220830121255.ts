...
export class BdtPeerClient extends EventEmitter{
    ...
    async downloadFileListener(session:string,interval:number,timeout:number): Promise<{err: ErrorCode,state?:string,record?:Array<{time:number,speed:string,progress:string}>}>{
        let begin = Date.now();
        let record:Array<{time:number,speed:string,progress:string}> = [{time:Date.now(),speed:"0",progress:"0"}];
 
        while(Date.now() - timeout < begin){
            let check = await this.downloadFileState(session);
