...
export class BdtPeerClient extends EventEmitter{
    ...
    async setFile(path:string,file:number): Promise<{err: ErrorCode, set_time?: number, file?: Buffer,ObjectId?:string}>{
        let info = await this.m_interface.callApi('sendBdtLpcCommand',  Buffer.from(""), {
            name: 'set-file',
            peerName: this.peerName,
            file,
    ...
    async uploadFile(path:string,file:number): Promise<{err: ErrorCode, set_time?: number, calculate_time?: number, file?: Buffer,ObjectId?:string}>{
        let info = await this.m_interface.callApi('sendBdtLpcCommand',  Buffer.from(""), {
            name: 'set-file',
            peerName: this.peerName,
            file,
