...
export class BdtPeerClient extends EventEmitter{
    ...
    async interestChunk(file:Buffer ,save_path:string,peer_id:string,second_peer_id?:string): Promise<{err: ErrorCode, file?: Buffer,session?:string}>{
    ...
    async checkChunk(session:string): Promise<{err: ErrorCode,state?:string}>{
        let info = await this.m_interface.callApi('sendBdtLpcCommand',  Buffer.from(""), {
