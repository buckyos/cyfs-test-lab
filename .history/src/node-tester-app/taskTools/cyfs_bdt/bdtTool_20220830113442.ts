...
export class BdtPeerClient extends EventEmitter{
    ...
    async calculateFile(path:string,chunk_size:number): Promise<{err: ErrorCode, calculate_time?: number, fileObj?: Buffer}>{
        let info = await this.m_interface.callApi('sendBdtLpcCommand',  Buffer.from(""), {
            name: 'calculate-file',
            peerName: this.peerName,
            chunk_size,
    ...
    async on_set_file(path:string,chunk_size:number): Promise<{err: ErrorCode, calculate_time?: number, fileObj?: Buffer}>{
        let info = await this.m_interface.callApi('sendBdtLpcCommand',  Buffer.from(""), {
            name: 'calculate-file',
            peerName: this.peerName,
            chunk_size,
