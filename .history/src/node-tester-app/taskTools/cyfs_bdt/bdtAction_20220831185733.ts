...
export class SendChunkAction extends ActionBase implements ActionAbstract {
    async run(): Promise<{ err: number, log: string }> {
        ...
        // (2) 构造测试数据
        // RN生成测试文件
        let randFile = await RN.bdtClient!.createFile(this.action.chunkSize!);
        // LN获取本地下载缓存文件路径
        let LNcachePath = await LN.bdtClient!.getCachePath();
        ...
        if (!this.action.config.not_wait_upload_finished) {
            ...
            if (setResult.err) {
                return { err: BDTERROR.success, log: `SendChunkAction run failed,err = ${JSON.stringify(setResult)},LN = ${this.action.LN},RN = ${this.action.RN}` }
            }
        }
        let download = await LN.bdtClient!.interestChunk(RN.bdtClient!.device_object!, calculate.chunk_id!);
        if (download.err) {
            return { err: BDTERROR.success, log: `SendChunkAction run failed,err = ${JSON.stringify(download)},LN = ${this.action.LN},RN = ${this.action.RN}` }
        }
        let check = await LN.bdtClient!.checkChunkListener(calculate.chunk_id!, 2000, this.action.config.timeout);
        if (check.err) {
            return { err: BDTERROR.success, log: `SendChunkAction run failed,err = ${JSON.stringify(check)},LN = ${this.action.LN},RN = ${this.action.RN}` }
        }
        let setResult = await setRunning;
        // (4) 校验结果
...
export class SendFileAction extends ActionBase implements ActionAbstract {
    async run(): Promise<{ err: number, log: string }> {
        ...
        let setResult = await setRunning;
        // (4) 校验结果
        let LN_hash = await LN.bdtClient!.md5File(savePath);
