...
export class ConnectAction extends ActionBase implements ActionAbstract {
    async run(): Promise<{ err: number, log: string }> {
        ...
        if(check.conn!.stream_name != info.conn!.stream_name){
            return { err: BDTERROR.connnetFailed, log: `${this.action.LN!} conenct ${this.action.RN!} , ${check.conn!.stream_name} != ${info.conn!.stream_name}` } 
        }
