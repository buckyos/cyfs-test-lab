...
export class BdtPeerClient extends EventEmitter{
    ...
    async remark_accpet_conn_name(TempSeq:string,remote:string,id:string,conn_tag?:string):Promise<{err:number,conn?:BdtConnection}>{
        for(let conn of this.m_conns.values()){
            if(conn.TempSeq == TempSeq && conn.remote == remote && conn.id == id){
