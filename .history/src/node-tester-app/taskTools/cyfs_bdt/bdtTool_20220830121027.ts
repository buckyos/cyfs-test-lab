...
export class BdtPeerClient extends EventEmitter{
    ...
    async downloadFileListener(session:string,interval:number,timeout:number): Promise<{err: ErrorCode,state?:string}>{
        let begin = Date.now();
        let record:Array<{time:number,speed:string,progress:string}> = [{time:Date.now(),speed:"0",progress:"0"}];
        let check : {err: ErrorCode,state?:string}
        while(Date.now() - timeout < begin){
            check = await this.downloadFileState(session);
        ...
        return {err: check.err,state:check.value?.state};
    }
}
