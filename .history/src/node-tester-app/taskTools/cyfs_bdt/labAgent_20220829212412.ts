...
import {NAT_Type,Agent} from "./type"
...
export type BdtPeerClientConfig={
    LW_type :string,
    eps:{ipv4:{tcp:boolean,udp:boolean},ipv4:{tcp:boolean,udp:boolean}}
}
export async function InitBdtPeerClientData(agent:Agent,config: BdtPeerClientConfig) {
    let epList = [];
    let LW_type = "L";
