...
export class BdtPeerClient extends EventEmitter{
    ...
    async downloadFile(file:Buffer ,save_path:string,peer_id:string,second_peer_id:string, ,chunk_size:number): Promise<{err: ErrorCode, set_time?: number, calculate_time?: number, file?: Buffer,ObjectId?:string}>{
        let info = await this.m_interface.callApi('sendBdtLpcCommand',  Buffer.from(""), {
